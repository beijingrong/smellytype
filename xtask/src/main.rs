//! Development tasks for smellytype
//!
//! Usage:
...
Commands:
  ...
  uninstall  Remove smellytype from /usr/local/bin (requires sudo)
...
Options:
  ...
  --no-stop-service
             For `dev`, do not stop systemd --user smellytype before launching
  -- ARGS    For `dev`, pass remaining args to target/release/smellytype

Examples:
  cargo xtask dev                # Build/run release+Vulkan dev daemon
  cargo xtask dev -- --verbose   # Pass args to smellytype
...
fn parse_dev_options(args: &[String]) -> anyhow::Result<DevOptions> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--" => {
                opts.smellytype_args.extend(args[i + 1..].iter().cloned());
                break;
            }
...
/// such as `smellytype-osd`, and those sidecars need to come from the same build
...
fn dev(args: &[String]) -> anyhow::Result<()> {
    ...
    let smellytype = release_dir.join("smellytype");
    if !smellytype.is_file() {
        anyhow::bail!("Binary not found at {}", smellytype.display());
    }

    if opts.stop_service {
        println!("==> Stopping systemd user service to avoid the single-instance lock...");
        let status = Command::new("systemctl")
            .args(["--user", "stop", "smellytype"])
            .stdin(Stdio::null())
            .status();
        match status {
            Ok(s) if s.success() => {}
            Ok(s) => eprintln!("warning: systemctl --user stop smellytype exited with {s}"),
    ...
    let bridge = release_dir.join("smellytype-audio-bridge");
    let style_file = default_style_file();

    println!("==> Running {}", smellytype.display());
    if opts.vulkan {
        println!("    feature: gpu-vulkan");
    }
    println!("    PATH starts with {}", release_dir.display());
    println!("    SMELLYTYPE_OSD_QML_PATH={}", qml_path.display());
    println!("    SMELLYTYPE_AUDIO_BRIDGE_BINARY={}", bridge.display());
    println!("    SMELLYTYPE_OSD_STYLE_FILE={}", style_file.display());
    println!();

    let status = Command::new(&smellytype)
        .args(&opts.smellytype_args)
        .current_dir(&root)
        .env("PATH", path)
        .env("SMELLYTYPE_OSD_QML_PATH", qml_path)
        .env("SMELLYTYPE_AUDIO_BRIDGE_BINARY", bridge)
        .env("SMELLYTYPE_OSD_STYLE_FILE", style_file)
        .status()?;
...
fn default_style_file() -> PathBuf {
    if let Ok(path) = env::var("SMELLYTYPE_OSD_STYLE_FILE") {
        if !path.is_empty() {
    ...
    runtime.join("smellytype/quickshell-style.json")
}
...
fn install(vulkan: bool) -> anyhow::Result<()> {
    ...
    let binary = root.join("target/release/smellytype");
    if !binary.exists() {
        anyhow::bail!("Binary not found at {:?}", binary);
    }

    println!("==> Installing to /usr/local/bin/smellytype...");
    ...
    println!();
    println!("Installed: /usr/local/bin/smellytype");

    // Show version
    let _ = Command::new("/usr/local/bin/smellytype")
        .arg("--version")
        .status();
...
/// Remove smellytype from /usr/local/bin
fn uninstall() -> anyhow::Result<()> {
    println!("==> Removing /usr/local/bin/smellytype...");

    let status = Command::new("sudo")
        .args(["rm", "-f", "/usr/local/bin/smellytype"])
        .status()?;
