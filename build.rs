...
fn main() -> Result<(), Error> {
    ...
    let generate = env::var("SMELLYTYPE_GEN_MANPAGES").is_ok() || profile == "release";
    ...
    // Generate main man page (smellytype.1)
    let man = Man::new(cmd.clone());
    let mut file = File::create(man_dir.join("smellytype.1"))?;
    man.render(&mut file)?;
    ...
    for subcommand in cmd.get_subcommands() {
        ...
        let mut file = File::create(man_dir.join(format!("smellytype-{}.1", name)))?;
        man.render(&mut file)?;

        // Generate man pages for nested subcommands (e.g., smellytype-setup-gpu)
        for nested in subcommand.get_subcommands() {
            ...
            let mut file =
                File::create(man_dir.join(format!("smellytype-{}-{}.1", name, nested_name)))?;
            man.render(&mut file)?;
        }
    }
...
fn expose_cuda_build_major() {
    ...
    println!("cargo:rustc-env=SMELLYTYPE_BUILD_CUDA_MAJOR={major}");
}
