# SmellyType Quickshell Frontend

A Quickshell/QML frontend for the smellytype daemon, shipped since v0.7.5 as an
alternative to the GTK4 (`smellytype-osd-gtk4`) and native (`smellytype-osd-native`)
...
| `EnginePicker.qml` | Floating panel listing every transcription engine, with the active one marked. Switches engines via `smellytype config set engine <name>`. Engines not compiled into the running binary are shown dimmed. |
| `MeetingControls.qml` | Meeting status panel (title, elapsed time, chunk count) with Start / Stop / Pause / Resume buttons that shell out to `smellytype meeting <action>`. |
| `smellytype-shared/` | Shared QML module: `Theme` (palette/sizing singleton), `StateReader` (daemon state file watcher), `AudioBridge` (wrapper around the `smellytype-audio-bridge` sidecar), `StyleLoader`, and `RecipeRenderer`. See [smellytype-shared/README.md](smellytype-shared/README.md). |

## Install

The packaged builds (AUR, deb, rpm) install the QML tree system-wide at
`/usr/share/smellytype/quickshell/` and the audio bridge at
`/usr/lib/smellytype/smellytype-audio-bridge`, so no extra step is needed there.
For source builds, or to get a per-user copy you can customize:

```bash
smellytype setup quickshell
```

This copies the QML files to `$XDG_DATA_HOME/smellytype/quickshell/`
(`~/.local/share/smellytype/quickshell/` by default), symlinks
`smellytype-audio-bridge` into `~/.local/bin/` so the waveform can find it on
PATH, and prints compositor binding examples for the popup widgets. See
`smellytype setup quickshell --help` for `--target`, `--source`, `--force`,
...
The `smellytype-osd-quickshell` launcher finds the installed QML tree
(`--qml-path` / `SMELLYTYPE_OSD_QML_PATH` override, then the per-user copy,
then `/usr/share/smellytype/quickshell/`, then `quickshell/` relative to the
current directory) and execs `qs -d -p <dir>`:

```bash
smellytype-osd-quickshell
```
...
Press your smellytype hotkey and watch the screen edge: the OSD card appears
...
under `$XDG_RUNTIME_DIR/smellytype/`, which the QML watches via `FileView`:

```bash
# Hyprland examples (see `smellytype setup quickshell --print-bindings`
# for Sway and River equivalents)
bind = SUPER, E, exec, mkdir -p $XDG_RUNTIME_DIR/smellytype && touch $XDG_RUNTIME_DIR/smellytype/engine-picker.flag
bind = SUPER, M, exec, mkdir -p $XDG_RUNTIME_DIR/smellytype && touch $XDG_RUNTIME_DIR/smellytype/meeting-controls.flag
```

## How it works

- **State**: `StateReader` watches `$XDG_RUNTIME_DIR/smellytype/state` with a
...
- **Audio**: Quickshell cannot read Unix domain sockets natively, so the
  `smellytype-audio-bridge` sidecar reads the daemon's audio socket
  (`$XDG_RUNTIME_DIR/smellytype/audio.sock`) and emits one NDJSON line per
...
- **Style**: `StyleLoader` reads launcher-resolved runtime JSON from
  `SMELLYTYPE_OSD_STYLE_FILE`, including Omarchy palette tokens, recipe layers,
...
- **No new daemon IPC**: the picker and meeting controls read existing
  ...
  commands (`smellytype config set`, `smellytype meeting <action>`,
  `smellytype info variants --json`).

## Documentation

- [User manual: `smellytype setup quickshell`](../docs/USER_MANUAL.md#smellytype-setup-quickshell)
...
Shareable style packages are directories with `smellytype-osd.toml`, optional
...
Same as the rest of smellytype (MIT).
