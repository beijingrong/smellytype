// SmellyType meeting controls popup for Quickshell.
//
...
// that shell out to the existing `smellytype meeting <action>` CLI
...
//
//   1. `$XDG_RUNTIME_DIR/smellytype/meeting_state` (FileView, watched) -
...
//      enabled.
//
//   2. `smellytype meeting show <id>` (Process, polled at 2 Hz while a
...
// `$XDG_RUNTIME_DIR/smellytype/meeting-controls.flag`. Bind it from your
// compositor like so:
//
//   # Hyprland
//   bind = SUPER, M, exec, touch $XDG_RUNTIME_DIR/smellytype/meeting-controls.flag
//
//   # Sway
//   bindsym $mod+m exec touch $XDG_RUNTIME_DIR/smellytype/meeting-controls.flag
//
...
// pattern used elsewhere in smellytype's runtime dir.
//
...
PanelWindow {
    ...
    /// Path or PATH-resolvable name of the smellytype CLI. AUR/.deb/.rpm
    /// packages install it as `/usr/bin/smellytype`; the bare name works
    /// for any standard install. Dev override:
    ///
    ///   MeetingControls { smellytypeBinary: "/path/to/target/release/smellytype" }
    ...
    /// Display title parsed from `smellytype meeting show`. Falls back
    ...
    /// `smellytype meeting show`). 0 while the show poll is in flight.
    ...
    WlrLayershell.namespace: "smellytype-meeting-controls"
    ...
    // ----- smellytype meeting show <id> poll (title + chunk count) -----

    Process {
        id: showProcess
        // Command is rebuilt every refresh because the meeting id is
        // dynamic and Process.command is read at start time.
        command: [root.smellytypeBinary, "meeting", "show", root._meetingId || "latest"]
        running: false
    ...
    // meeting ends so an idle smellytype install doesn't spawn a
    ...
    Process {
        id: startProcess
        command: [root.smellytypeBinary, "meeting", "start"]
        running: false
    ...
    Process {
        id: stopProcess
        command: [root.smellytypeBinary, "meeting", "stop"]
        running: false
    ...
    Process {
        id: pauseProcess
        command: [root.smellytypeBinary, "meeting", "pause"]
        running: false
    ...
    Process {
        id: resumeProcess
        command: [root.smellytypeBinary, "meeting", "resume"]
        running: false
    ...
    function _parseShowOutput(buf) {
        // Parses the human-readable `smellytype meeting show` output.
    ...
    // styling, lift it into smellytype-shared.
