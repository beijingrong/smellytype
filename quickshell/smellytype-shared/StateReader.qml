// SmellyType daemon state file watcher.
//
// Wraps Quickshell.Io.FileView around the daemon's state file at
// $XDG_RUNTIME_DIR/smellytype/state. The file contains exactly one of
...
//           console.log("smellytype is now", newState);
//       }
//   }
//
// The component also exposes `osdSuppressed`, set while the daemon is
// running a recording started with `smellytype record start --no-osd`. OSD
