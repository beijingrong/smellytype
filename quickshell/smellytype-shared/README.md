...
Shared Quickshell QML modules used by every smellytype QML frontend: the
...
Watches `$XDG_RUNTIME_DIR/smellytype/state` for changes and exposes the
...
| Property    | Default                                  | Purpose                              |
|-------------|------------------------------------------|--------------------------------------|
| `statePath` | `$XDG_RUNTIME_DIR/smellytype/state`         | Override for tests / custom setups   |
...
Wraps the `smellytype-audio-bridge` sidecar binary. The bridge reads the
...
| Property         | Default                  | Purpose                                                            |
|------------------|--------------------------|--------------------------------------------------------------------|
| `bridgeBinary`   | `smellytype-audio-bridge`   | Path or PATH-resolvable name of the sidecar                        |
...
Reads the runtime style JSON path from `SMELLYTYPE_OSD_STYLE_FILE`. The
`smellytype-osd-quickshell` launcher writes this file after resolving
...
These modules back the three Quickshell widgets that ship with smellytype
