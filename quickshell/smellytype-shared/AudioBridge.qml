// SmellyType audio-bridge wrapper.
//
// Spawns the `smellytype-audio-bridge` sidecar binary as a child Process and
// parses one NDJSON object per line of stdout. The sidecar reads from
// the daemon's audio Unix socket (`$XDG_RUNTIME_DIR/smellytype/audio.sock`)
...
Item {
    ...
    /// AUR/.deb/.rpm packages install it as `smellytype-audio-bridge` in
    /// `/usr/bin`, so the default works for end-user installs. For
    /// development, override with the cargo target path:
    ///
    ///   VT.AudioBridge {
    ///       bridgeBinary: "/path/to/target/release/smellytype-audio-bridge"
    ///   }
    property string bridgeBinary: "smellytype-audio-bridge"
    ...
    function _handleLine(line) {
        ...
        try {
            ...
            console.warn("smellytype audio-bridge: non-JSON stdout line:", trimmed);
            return;
        }
