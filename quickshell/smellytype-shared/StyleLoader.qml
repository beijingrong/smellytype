...
//
// `smellytype-osd-quickshell` writes a JSON file and exposes it through
// SMELLYTYPE_OSD_STYLE_FILE. This component parses it and falls back to the
...
QtObject {
    ...
    property string styleFile: Quickshell.env("SMELLYTYPE_OSD_STYLE_FILE") || ""
    property var config: _defaultConfig()

    // Degraded-path fallback used only when SMELLYTYPE_OSD_STYLE_FILE is
    ...
    function _loadText(text) {
        try {
            ...
            console.warn("smellytype OSD style: invalid JSON:", e);
