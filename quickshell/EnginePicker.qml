// SmellyType engine picker popup for Quickshell.
//
// A floating panel that lists every transcription engine smellytype knows
// about, marks the one currently selected in the on-disk config, and
// switches the active engine by shelling out to
// `smellytype config set engine <name>` (added in PR #382). No new daemon
// IPC is introduced.
//
// ## Read path
//
...
//    feature is discovered by spawning `smellytype info variants --json`
...
//    `smellytype config set engine`'s feature gate to reject a request
...
//    `~/.config/smellytype/config.toml` via FileView. The file is parsed
...
// `$XDG_RUNTIME_DIR/smellytype/engine-picker.flag`. Touching the flag
// toggles visibility; the picker removes the flag on read so a
// subsequent `touch` reliably retoggles.
//
//   # Hyprland
//   bind = SUPER, E, exec, touch $XDG_RUNTIME_DIR/smellytype/engine-picker.flag
//
//   # Sway
//   bindsym $mod+e exec touch $XDG_RUNTIME_DIR/smellytype/engine-picker.flag
//
// ## Switching
//
// On Enter or click, the picker spawns
// `smellytype config set engine <name>`. Once the process exits, the
...
PanelWindow {
    ...
    /// Path or PATH-resolvable name of the smellytype CLI. Mirrors
    /// MeetingControls so a single override flips both widgets.
    property string smellytypeBinary: "smellytype"
    ...
    property string runtimeDir: {
        ...
        if (xdg && xdg.length > 0) {
            return xdg + "/smellytype";
        }
        const uid = Quickshell.env("UID");
        if (uid && uid.length > 0) {
            return "/run/user/" + uid + "/smellytype";
        }
        return "/run/user/1000/smellytype";
    }

    /// Path to the on-disk config file. Mirrors
    /// `Config::default_path()`: `$XDG_CONFIG_HOME/smellytype/config.toml`
    /// with a `$HOME/.config/smellytype/config.toml` fallback.
    property string configPath: {
        const xdgConfig = Quickshell.env("XDG_CONFIG_HOME");
        if (xdgConfig && xdgConfig.length > 0) {
            return xdgConfig + "/smellytype/config.toml";
        }
        const home = Quickshell.env("HOME");
        if (home && home.length > 0) {
            return home + "/.config/smellytype/config.toml";
        }
        return "/tmp/smellytype-config.toml";
    }
    ...
    /// `smellytype info variants --json` `compiled_features`. Used to
    ...
    WlrLayershell.namespace: "smellytype-engine-picker"
    ...
    // ----- `smellytype info variants --json` (compiled features) -----

    Process {
        id: featuresProcess
        command: [root.smellytypeBinary, "info", "variants", "--json"]
        running: false
    ...
    // ----- `smellytype config set engine <name>` (switch) -----
    ...
    Process {
        ...
        command: [root.smellytypeBinary, "config", "set", "engine", "whisper"]
        running: false
    ...
    // between opens via `smellytype-bin` package upgrades.
    onOpenChanged: {
        if (root.open) {
            configFile.reload();
    ...
    function _parseEngineFromToml(content) {
        ...
        // level. The setting lives on the root table in smellytype's
    ...
    function _parseFeaturesJson(buf) {
        ...
        try {
            ...
            // Older smellytype binaries that predate `info variants
    ...
    function _commit() {
        ...
        switchProcess.command = [
            root.smellytypeBinary, "config", "set", "engine", name
        ];
    ...
    function _handleSwitchExit(stderrText) {
        ...
        if (root._activeEngine === name) {
            root._actionKind = "ok";
            root._actionStatus = "Switched to " + name + ". Restart smellytype to apply.";
            ...
            root._actionKind = "error";
            root._actionStatus = "Failed to write config (see smellytype logs).";
            actionStatusTimer.restart();
        }
        root._pendingEngine = "";
    }
    ...
    // EnginePicker.qml stays a single file. Lift into smellytype-shared
