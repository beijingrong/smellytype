...
```bash
mkdir -p "$XDG_RUNTIME_DIR/smellytype"
printf recording > "$XDG_RUNTIME_DIR/smellytype/state"

cargo run --bin smellytype-osd-quickshell -- \
  --qml-path quickshell \
...
VoxType recording so `smellytype-audio-bridge` receives frames from the daemon.

Hide the OSD:

```bash
printf idle > "$XDG_RUNTIME_DIR/smellytype/state"
```

## Package Shape

- `smellytype-osd.toml`: package metadata, palette, fallback recipe data, and the
