#!/bin/bash
# Example post-processing script for SmellyType using OpenAI API
#
# Usage: Configure in ~/.config/smellytype/config.toml:
#   [output.post_process]
...
if [[ -n "${SMELLYTYPE_CONTEXT:-}" ]]; then
  JSON=$(jq -n --arg text "$INPUT" --arg system "$SYSTEM_PROMPT" --arg context "$SMELLYTYPE_CONTEXT" '{
    model: "gpt-4o-mini",
    messages: [
