...
use smellytype::config::{Config, VadBackend, VadConfig};
use smellytype::vad::{create_vad, EnergyVad, VoiceActivityDetector};
...
fn create_vad_returns_energy_vad_for_parakeet() {
    ...
    config.engine = smellytype::config::TranscriptionEngine::Parakeet;
...
fn try_create_whisper_vad() -> Option<smellytype::vad::WhisperVad> {
    ...
    smellytype::vad::WhisperVad::new(&model_path, &config).ok()
}
