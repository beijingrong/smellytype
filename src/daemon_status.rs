//! Single source of truth for "is the smellytype daemon running?" across every
//! external caller — the CLI (`smellytype record`, `smellytype meeting`,
//! `smellytype status`), the TUI (`smellytype configure`), and any future
//! integration point.
//!
...
//!   `send_record_command()` read `runtime_dir/smellytype.lock`, breaking
//!   `smellytype meeting start/stop/pause/resume` (the daemon was healthy but
...
pub fn pid_file_path() -> std::path::PathBuf {
    Config::runtime_dir().join("smellytype.lock")
}
...
/// smellytype daemon could never legitimately be, so reject it too.
///
...
/// running, start with: smellytype daemon" message and exits the process if
/// the daemon is down.
///
/// Callers that don't need the PID (e.g. `smellytype meeting status`) can
...
pub fn check_daemon_running() -> anyhow::Result<i32> {
    ...
    if !pid_file.exists() {
        eprintln!("Error: SmellyType daemon is not running.");
        eprintln!("Start it with: smellytype daemon");
        std::process::exit(1);
    }
    ...
    // PID 1 is init/systemd which a user-mode smellytype daemon could never
    // legitimately be. See the rationale on `read_pid`.
    if pid <= 1 {
        let _ = std::fs::remove_file(&pid_file);
        eprintln!(
            "Error: SmellyType daemon is not running (lockfile held an invalid PID, removed)."
        );
        eprintln!("Start it with: smellytype daemon");
        std::process::exit(1);
    }
    ...
    if !is_running(pid) {
        ...
        let _ = std::fs::remove_file(&pid_file);
        eprintln!("Error: SmellyType daemon is not running (stale PID file removed).");
        eprintln!("Start it with: smellytype daemon");
        std::process::exit(1);
    }
...
mod tests {
    ...
    /// `send_record_command()` read `runtime_dir/smellytype.lock`. The mismatch
    /// caused `smellytype meeting start/stop/pause/resume` to falsely report
    ...
    fn pid_file_path_matches_send_record_command() {
        ...
        // Sanity: the canonical path ends in `smellytype.lock` (the Pidlock file
        // the daemon actually writes), not the legacy `pid` filename.
        assert!(
            canonical.ends_with("smellytype.lock"),
            "pid_file_path() must point at smellytype.lock so meeting \
             and record commands agree with the daemon's Pidlock. Got: {:?}",
            canonical,
        );

        // Whatever path the daemon writes (`Config::runtime_dir/smellytype.lock`)
        // must match what every external caller reads.
        let from_send = Config::runtime_dir().join("smellytype.lock");
