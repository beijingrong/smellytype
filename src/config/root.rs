use super::{
    AudioConfig, CohereConfig, DolphinConfig, DoubaoConfig, HotkeyConfig, MeetingConfig,
...
pub struct Config {
    ...
    /// daemon spawns the `smellytype-osd` child and how it renders.
    #[serde(default)]
    ...
    /// Example: "/run/user/1000/smellytype/state" or use "auto" for default location
    #[serde(default = "default_state_file")]
    pub state_file: Option<String>,

    /// Named profiles for context-specific settings
    /// Example: [profiles.slack], [profiles.code]
    /// Use with: `smellytype record start --profile slack`
    #[serde(default)]
...
impl Config {
    ...
    pub const SYSTEM_PATH: &'static str = "/etc/smellytype/config.toml";
    ...
    /// 1. User config (`~/.config/smellytype/config.toml`)
    /// 2. System-wide config (`/etc/smellytype/config.toml`)
    ///
    ...
    /// SmellyType's user config directory, honoring `$XDG_CONFIG_HOME` (default
    ...
    pub fn config_dir() -> Option<PathBuf> {
        Self::xdg_dir(
            "XDG_CONFIG_HOME",
            ".config",
            directories::ProjectDirs::from("", "", "smellytype")
                .map(|d| d.config_dir().to_path_buf()),
        )
    }

    /// SmellyType's user data directory (parent of the models dir), honoring
    ...
    /// Resolve a `smellytype` user dir. An explicit absolute `$xdg_var` wins;
    /// otherwise `$HOME/<default_rel>/smellytype`. Falls back to an existing
    ...
    fn xdg_dir(xdg_var: &str, default_rel: &str, legacy: Option<PathBuf>) -> Option<PathBuf> {
        ...
        {
            return Some(base.join("smellytype"));
        }
        let xdg = std::env::var_os("HOME")
            .filter(|h| !h.is_empty())
            .map(|h| PathBuf::from(h).join(default_rel).join("smellytype"));
        match (xdg, legacy) {
    ...
    pub fn on_demand_loading(&self) -> bool {
        match self.engine {
            ...
            TranscriptionEngine::Soniox | TranscriptionEngine::Doubao => false,
        }
    }
    ...
    pub fn model_name(&self) -> &str {
        match self.engine {
            ...
            TranscriptionEngine::Doubao => "doubao-streaming-2.0",
        }
    }
...
mod tests {
    ...
    fn test_system_path_constant() {
        assert_eq!(
            Config::system_path(),
            PathBuf::from("/etc/smellytype/config.toml")
        );
        assert_eq!(Config::SYSTEM_PATH, "/etc/smellytype/config.toml");
    }
    ...
    fn xdg_dir_resolution() {
        ...
        let xdg = tmp.path().join(".config/smellytype");
        let legacy = tmp.path().join("Library/Application Support/smellytype");
        ...
        std::env::set_var("XDG_CONFIG_HOME", "/tmp/smellytype-xdg-abs");
        assert_eq!(
            Config::config_dir(),
            Some(PathBuf::from("/tmp/smellytype-xdg-abs/smellytype"))
        );
