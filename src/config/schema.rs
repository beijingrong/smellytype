...
//!
//! This is the single source of truth shared by `smellytype config set/unset/get`,
//! `smellytype config schema --json`, and any external settings panel (the
//! Omarchy/Quickshell panel is the first consumer). Entries mirror the rows
//! the `smellytype configure` TUI renders, so the two surfaces can't disagree
...
//!    never reads — smellytype's config has no `deny_unknown_fields`, so a
...
/// Version of the `smellytype config schema --json` document shape. Bump when
...
/// human-readable `smellytype config schema` table.
...
pub enum KeyType {
    ...
    /// `smellytype info <source> --json`. Never rejected at set time, because
...
/// Engines `smellytype config set engine` accepts. Mirrors `ENGINE_CHOICES` in
...
/// means absent. Use `smellytype config unset osd.palette`.
...
pub const CONFIG_KEYS: &[KeySpec] = &[
    ...
    // settable with no way to say what it switches to. The `smellytype configure`
    ...
    spec(
        ...
        "Engine",
        "Model",
        "Parakeet model directory name under the smellytype models dir.",
    )
    ...
    spec(
        ...
        "Engine",
        "Threads",
        "ONNX Runtime intra-op threads. Unset lets smellytype pick.",
    )
    .for_onnx_engine("moonshine"),
    spec(
    ...
    spec(
        ...
        "Engine",
        "Threads",
        "CPU inference threads. Unset lets smellytype pick.",
    )
    .for_onnx_engine("openvino"),
    spec(
        "openvino.language",
    ...
    spec(
        ...
        "Watch the keyboard via evdev. Turn this off when your compositor calls `smellytype record` instead.",
    ),
    spec(
        "hotkey.key",
        "hotkey",
    ...
    // Cloud settings exposed to the companion panel, ahead of the stock TUI.
    spec(
    ...
    spec(
        ...
        "Meeting",
        "Meeting mode",
        "Allow `smellytype meeting` to record and transcribe long sessions.",
    ),
    spec(
    ...
    spec(
        ...
        "Status",
        "Icon theme",
        "Glyph set used by `smellytype status` for Waybar and tray integrations.",
    )
    .live(),
    spec(
        "status.icons.idle",
...
pub enum ValueError {
    ...
    #[error(
        "{key} cannot be set to an empty string. Use `smellytype config unset {key}` instead."
    )]
...
/// Build the `smellytype config schema --json` document.
pub fn schema_json(cfg: &Config, path: &Path, editor: &ConfigEditor) -> Json {
    ...
    let replacements: Map<String, Json> = cfg
        ...
        .map(|(k, v)| (k.clone(), json!(v)))
        .collect();
    // `smellytype_version` is this CLI's build. `daemon_version` is what is
    ...
    json!({
        ...
        "smellytype_version": env!("CARGO_PKG_VERSION"),
...
/// Build the `smellytype config get --json` document: every allowlisted key
...
mod tests {
    ...
    /// smellytype's config structs don't use `deny_unknown_fields`, so writing
    ...
    fn onnx_engine_keys_are_feature_gated() {
        for s in CONFIG_KEYS {
            ...
            if matches!(engine, "whisper" | "doubao") {
                assert!(
                    s.requires_feature.is_none(),
                    "{} should not be feature-gated; this backend is always compiled in",
                    s.key
                );
            } else {
                assert_eq!(
    ...
    fn schema_json_has_the_documented_envelope() {
        ...
        assert_eq!(doc["smellytype_version"], json!(env!("CARGO_PKG_VERSION")));
