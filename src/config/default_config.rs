...
pub const DEFAULT_CONFIG: &str = r#"# SmellyType Configuration
#
# Location: ~/.config/smellytype/config.toml
# All settings can be overridden via CLI flags

# State file for external integrations (Waybar, polybar, etc.)
# Use "auto" for default location ($XDG_RUNTIME_DIR/smellytype/state),
...
# Required for `smellytype record toggle` and `smellytype status` commands.
state_file = "auto"
...
# When disabled, use `smellytype record start/stop/toggle` to control recording
# enabled = true
...
# List devices with: smellytype info devices
...
# Auto-stop an external-trigger recording (`smellytype record start`, e.g. from
...
# NVIDIA dGPU), use SMELLYTYPE_VULKAN_DEVICE=nvidia instead (simpler).
...
#
# API key for remote server (optional, or use SMELLYTYPE_WHISPER_API_KEY env var)
# remote_api_key = ""
#
...
# replacements = { "vox type" = "smellytype" }
#
...
# plugin_path = "/path/to/smellytype-osd-package"
