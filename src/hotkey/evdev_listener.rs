    ...
    fn try_open_device(&mut self, path: &PathBuf) {
        match Device::open(path) {
            Ok(device) => {
                ...
                // (dotool, ydotool, xdotool). smellytype types its transcription out
...
/// (dotool, ydotool, wtype, xdotool). smellytype types its transcription out through
...
mod tests {
    ...
    fn poll_guard_drops_real_torn_down_evdev_device() {
        ...
        // real smellytype instance's keyboard filter ignores it and is unaffected by
        ...
        let builder = builder.name("smellytype-test-nonkbd");
    ...
    fn synced_fetch_recovers_multiple_held_keys_after_overflow() {
        ...
        let builder = builder.name("smellytype-test-dotool-sync-overflow");
        let builder = match builder.with_keys(&keys) {
            Ok(b) => b,
            Err(e) => {
                eprintln!("skipping: with_keys failed ({e})");
