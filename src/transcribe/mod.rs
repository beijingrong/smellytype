...
pub fn create_transcriber(config: &Config) -> Result<Box<dyn Transcriber>, TranscribeError> {
    match config.engine {
        ...
        TranscriptionEngine::Parakeet => Err(TranscribeError::InitFailed(
            "Parakeet engine requested but smellytype was not compiled with --features parakeet"
                .to_string(),
        )),
        #[cfg(feature = "moonshine")]
        ...
        TranscriptionEngine::Moonshine => Err(TranscribeError::InitFailed(
            "Moonshine engine requested but smellytype was not compiled with --features moonshine"
                .to_string(),
        )),
        #[cfg(feature = "sensevoice")]
        ...
        TranscriptionEngine::SenseVoice => Err(TranscribeError::InitFailed(
            "SenseVoice engine requested but smellytype was not compiled with --features sensevoice"
                .to_string(),
        )),
        #[cfg(feature = "paraformer")]
        ...
        TranscriptionEngine::Paraformer => Err(TranscribeError::InitFailed(
            "Paraformer engine requested but smellytype was not compiled with --features paraformer"
                .to_string(),
        )),
        #[cfg(feature = "dolphin")]
        ...
        TranscriptionEngine::Dolphin => Err(TranscribeError::InitFailed(
            "Dolphin engine requested but smellytype was not compiled with --features dolphin"
                .to_string(),
        )),
        #[cfg(feature = "omnilingual")]
        ...
        TranscriptionEngine::Omnilingual => Err(TranscribeError::InitFailed(
            "Omnilingual engine requested but smellytype was not compiled with --features omnilingual"
                .to_string(),
        )),
        #[cfg(feature = "cohere")]
        ...
        TranscriptionEngine::Cohere => Err(TranscribeError::InitFailed(
            "Cohere engine requested but smellytype was not compiled with --features cohere"
                .to_string(),
        )),
        TranscriptionEngine::Doubao => {
            Ok(Box::new(doubao::DoubaoTranscriber::new(config.doubao.clone().unwrap_or_default())?))
        }
        ...
        TranscriptionEngine::OpenVino => Err(TranscribeError::InitFailed(
            "OpenVINO engine requested but smellytype was not compiled with --features openvino-whisper"
                .to_string(),
        )),
    }
}
...
pub fn create_transcriber_with_config_path(
    ...
    // Apply GPU selection from SMELLYTYPE_VULKAN_DEVICE environment variable
