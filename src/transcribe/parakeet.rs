...
fn probe_cuda_runtime() -> bool {
    ...
    // mirrors that selection into SMELLYTYPE_BUILD_CUDA_MAJOR so this probe
    ...
    //
    // SmellyType ships separate smellytype-onnx-cuda-12 and smellytype-onnx-cuda-13
    // binaries. `smellytype setup gpu --enable` symlinks smellytype-onnx-cuda to
    ...
    // ORT_CUDA_VERSION=13 in its Dockerfile so SMELLYTYPE_BUILD_CUDA_MAJOR
    ...
    {
        const EXPECTED_CUDA_MAJOR: i32 = match env!("SMELLYTYPE_BUILD_CUDA_MAJOR").as_bytes() {
            b"13" => 13,
            _ => 12,
        };
        ...
        if major != EXPECTED_CUDA_MAJOR {
            tracing::error!(
                "CUDA version mismatch: found CUDA {major}.{minor}, but this binary's \
                 ...
                 Options:\n  \
                 1. Install the matching smellytype-onnx-cuda-{EXPECTED_CUDA_MAJOR} package\n  \
                 2. Switch to smellytype-onnx-cuda-{} for your CUDA version (`smellytype setup gpu --enable` \
