...
/// init. Same `$XDG_CACHE_HOME/smellytype/<name>` convention as the MIGraphX
    ...
    if !encoder_xml.exists() {
        return Err(TranscribeError::ModelNotFound(format!(
            "OpenVINO Whisper encoder model not found: {}\n  \
             Run 'smellytype setup model' to download, or manually from:\n  \
...
/// available, but that would let `smellytype setup model` claim that NPU setup
    ...
    fn require_preprocessor_config(
        ...
        Err(TranscribeError::ModelNotFound(format!(
            "OpenVINO Whisper model at {} is missing preprocessor_config.json. \
             ...
             smellytype setup --download --model {}",
            model_dir.display(),
            model_name,
        )))
    }
...
impl Transcriber for OpenVinoTranscriber {
    ...
    fn transcribe(&self, samples: &[f32]) -> Result<String, TranscribeError> {
        ...
        if is_multilingual {
            ...
            // in generation_config.json), while smellytype config uses bare codes like "en"
...
/// - Short names with quantization: "base.en-int8" (from `smellytype setup model`)
...
fn resolve_model_path(model: &str, quantized: bool) -> Result<PathBuf, TranscribeError> {
    ...
    // Names from `smellytype setup model` include quantization (e.g., "base.en-int8").
...
mod tests {
    ...
    fn incomplete_model_fails_before_openvino_first_inference() {
        ...
        assert!(error.contains("smellytype setup --download --model medium-int4"));
    ...
    fn test_resolve_no_double_quant_suffix() {
        // When model name already has quantization (from `smellytype setup model`),
    ...
    /// Requires: model files in ~/.local/share/smellytype/models/, OpenVINO GenAI libs, NPU device.
    ...
    fn test_openvino_real_transcription() {
        ...
        let device =
            std::env::var("SMELLYTYPE_OPENVINO_DEVICE").unwrap_or_else(|_| "CPU".to_string());
        let model =
            std::env::var("SMELLYTYPE_OPENVINO_MODEL").unwrap_or_else(|_| "base".to_string());
        let config = OpenVinoConfig {
            model,
            device: device.clone(),
            quantized: true,
            openvino_dir: std::env::var("SMELLYTYPE_OPENVINO_DIR").ok(),
            ..OpenVinoConfig::default()
        };
