...
//! be changed by later messages). SmellyType's `StreamingSession` types
...
/// smellytype `StreamingEvent` deltas.
///
/// Soniox emits cumulative finals (committed forever) and revisable
/// non-finals. SmellyType's `StreamingSession` types events as deltas at the
...
/// Soniox sessions are single-segment from smellytype's perspective: the WS
...
impl Transcriber for SonioxTranscriber {
    ...
    /// runtime. SmellyType's binary entry point is multi-thread so this is
    ...
    fn transcribe(&self, samples: &[f32]) -> Result<String, TranscribeError> {
        ...
        //
        // - One-shot CLI (`smellytype transcribe file.wav`): no ambient
...
impl SonioxTranscriber {
    async fn batch_transcribe(&self, samples: &[f32]) -> Result<String, TranscribeError> {
        ...
        tracing::debug!(target: "smellytype::soniox::wire", "-> init {}", redact_api_key(&init));
...
impl SonioxTranscriber {
    async fn async_transcribe(&self, samples: &[f32]) -> Result<String, TranscribeError> {
        ...
        tracing::debug!(target: "smellytype::soniox::wire", "<- transcript: {}", body);
...
async fn run_streaming_session(
    ...
    // Send init.
    tracing::debug!(target: "smellytype::soniox::wire", "-> init {}", redact_api_key(&init_frame));
    ...
    loop {
        ...
        tokio::select! {
            ...
            msg = read.next() => {
                ...
                tracing::debug!(target: "smellytype::soniox::wire", "<- {}", text);
                let parsed: ServerMessage = match serde_json::from_str(&text) {
                    Ok(p) => p,
                    Err(e) => {
...
mod tests {
    ...
    fn load_context_terms_dedupes_and_trims() {
        ...
        std::fs::write(
            &path,
            r#"["Claude", "  smellytype  ", "Claude", "", "Hyprland"]"#,
        )
        .unwrap();
        let mut cfg = cfg_with_key(Some("k"));
        cfg.terms = Some(vec!["smellytype".into(), "Codecool".into()]);
        cfg.terms_file = Some(path);
        let t = SonioxTranscriber::new(cfg).unwrap();
        assert_eq!(
            t.context_terms,
            vec!["smellytype", "Codecool", "Claude", "Hyprland"],
        );
    }
