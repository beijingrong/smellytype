...
///
/// Spawns a fresh `smellytype transcribe-worker` process for each transcription.
...
impl SubprocessTranscriber {
    ...
    /// Get the path to the smellytype executable
    fn get_executable_path() -> Result<std::path::PathBuf, TranscribeError> {
        std::env::current_exe().map_err(|e| {
            TranscribeError::InitFailed(format!("Cannot find smellytype executable: {}", e))
        })
    }
