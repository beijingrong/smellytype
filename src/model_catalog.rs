...
//!
//! Extracted from `src/tui/engine.rs` so `smellytype info models --json` and the
...
/// Models smellytype knows how to download for `engine`.
...
/// Default model name baked into smellytype for each engine. Used when a fresh
...
///
/// `None` means `smellytype setup --download` can't fetch it: `run_setup` only
/// routes Whisper, Parakeet, SenseVoice, and OpenVINO names, so the other
/// ONNX engines
/// are reachable only through the interactive picker (`smellytype setup model`).
...
///
/// - ONNX models downloaded since smellytype started caching manifests carry the
...
///
/// Reads every byte, so this is only for `smellytype info models --verify`, never
/// the default listing.
...
mod tests {
    ...
    /// Models installed before smellytype cached manifests must not be called
