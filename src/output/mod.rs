...
/// Path to the smellytype symlink
const SMELLYTYPE_BIN: &str = "/usr/lib/smellytype/smellytype";

/// Check if the active binary is a Parakeet build
pub fn is_parakeet_binary_active() -> bool {
    if let Ok(link_target) = fs::read_link(SMELLYTYPE_BIN) {
...
pub fn engine_icon(engine: crate::config::TranscriptionEngine) -> &'static str {
    match engine {
        ...
        crate::config::TranscriptionEngine::Soniox | crate::config::TranscriptionEngine::Doubao => {
            "\u{2601}\u{FE0F}"
        } // ☁️
...
pub async fn send_transcription_notification(
    ...
    // that module owns the --replace-id bookkeeping that keeps every SmellyType
