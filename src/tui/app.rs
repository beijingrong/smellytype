...
pub enum Action {
    ...
    ForceQuit,
    /// Move /usr/bin/smellytype to the named variant via pkexec.
    SwitchVariant(Variant),
    /// Run `smellytype setup --download --model <model>` to download a missing
    /// model. The
...
pub struct App {
    ...
    /// but `/usr/bin/smellytype` dispatches to the CPU Whisper variant). When
    ...
    /// `restart_smellytype_daemon` call. Compared on quit — a newer mtime
...
/// `/usr/lib/smellytype/` on every refresh.
...
fn detect_missing_model() -> Option<MissingModel> {
    ...
    let (engine_name, model, setup_command) = match cfg.engine {
        ...
        config::TranscriptionEngine::Soniox | config::TranscriptionEngine::Doubao => return None,
