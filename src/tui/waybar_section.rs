...
fn guidance(state: &WaybarState) -> Vec<Line<'static>> {
    match state.field {
        Field::Theme => vec![
            ...
            Line::from(""),
            Line::from(
                "The glyph set `smellytype status --follow` emits to your status \
            ...
            Line::from(""),
            Line::from(Span::styled(
                "Run `smellytype setup waybar` for ready-to-paste Waybar config.",
        ...
        Field::IconIdle => icon_guidance(
            state,
            "idle",
            "Shown when smellytype is loaded but not actively recording or \
             transcribing.",
        ),
        Field::IconRecording => icon_guidance(
            state,
            "recording",
            "Shown while smellytype is capturing audio.",
        ),
        Field::IconTranscribing => icon_guidance(
            state,
            "transcribing",
            "Shown while smellytype is running inference on a captured clip.",
        ),
        Field::IconStopped => icon_guidance(
            state,
            "stopped",
            "Shown when the daemon isn't running. Useful for spotting that \
             smellytype crashed or wasn't started.",
        ),
    }
}
