        ...
        Field::Backend => vec![
            ...
            Line::from(
                "Silero VAD via whisper-rs. Most accurate. Requires \
                 ggml-silero-vad.bin (run `smellytype setup vad` to fetch).",
            ),
        ],
        Field::Threshold => vec![
            ...
            Line::from(""),
            Line::from(
                "0.0-1.0. Higher values mean smellytype demands more confident \
                 speech detection before transcribing.",
            ),
            Line::from(""),
            Line::from(
                "0.5 is the default and works for most setups. Bump higher \
                 (0.65-0.75) if smellytype occasionally transcribes background \
