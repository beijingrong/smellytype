//! Detect `smellytype record` bindings declared in compositor configs.
//!
//! Useful when the user has the evdev listener disabled and is relying on
//! compositor-level keybindings to call smellytype. The Hotkey section's About
...
//!
//! - River: shell-script-based init; any function could call smellytype, so a
...
pub struct Binding {
    ...
    pub keys: String,
    /// SmellyType subcommand being bound (`record start`, `record cancel`,
    ...
    if streaming {
        ...
        if has_start && !has_stop {
            suggestions.push(make_suggestion(
                ...
                "Without a stop binding, hold-to-record never finishes — smellytype \
    ...
    if !has_meeting_start && !has_meeting_stop {
        ...
        suggestions.push(make_suggestion(
            ...
            "You have a meeting-start binding but no stop. Without it the \
             meeting only ends when you run `smellytype meeting stop` from the CLI.",
            Role::MeetingStop,
        ));
    ...
    match (comp, role) {
        (Compositor::Hyprland, Role::Start) => {
            vec![hyprland_bind(
                "bindd",
                key,
                "SmellyType PTT (start)",
                "smellytype record start",
            )]
        }
        ...
        (Compositor::Sway, Role::Start) => {
            vec![format!(
                "bindsym {} exec smellytype record start",
                canonical_to_sway(key)
            )]
        }
        (Compositor::Sway, Role::Stop) => vec![format!(
            "bindsym --release {} exec smellytype record stop",
            canonical_to_sway(key)
        )],
        ...
        (Compositor::Sway, Role::Toggle) => vec![format!(
            "bindsym {} exec smellytype record toggle",
            canonical_to_sway(key)
        )],
        (Compositor::Sway, Role::Cancel) => vec![format!(
            "bindsym {} exec smellytype record cancel",
            canonical_to_sway(key)
        )],
        (Compositor::Sway, Role::MeetingStart) => vec![format!(
            "bindsym {} exec smellytype meeting start",
            canonical_to_sway(key)
        )],
        (Compositor::Sway, Role::MeetingStop) => vec![format!(
            "bindsym {} exec smellytype meeting stop",
            canonical_to_sway(key)
        )],
        ...
        (Compositor::Niri, Role::PttPair) => vec![
            format!(
                "{} {{ spawn \"smellytype\" \"record\" \"toggle\"; }}",
                canonical_to_niri(key)
            ),
        ...
        (Compositor::Niri, Role::Cancel) => vec![format!(
            "{} {{ spawn \"smellytype\" \"record\" \"cancel\"; }}",
            canonical_to_niri(key)
        )],
        (Compositor::Niri, Role::MeetingStart) => vec![format!(
            "{} {{ spawn \"smellytype\" \"meeting\" \"start\"; }}",
            canonical_to_niri(key)
        )],
        (Compositor::Niri, Role::MeetingStop) => vec![format!(
            "{} {{ spawn \"smellytype\" \"meeting\" \"stop\"; }}",
            canonical_to_niri(key)
        )],
...
/// Hyprland `bindd? = MODS, KEY, NAME, exec, smellytype SUBCMD ACTION` lines
...
/// Sway `bindsym MOD+KEY exec smellytype SUBCMD ACTION` (or `bindcode`).
fn parse_sway_line(line: &str, source: &Path) -> Option<Binding> {
    let trimmed = line.trim();
    if trimmed.starts_with('#') {
        return None;
    }
    if !trimmed.contains("smellytype") {
        return None;
    }
    let mut parts = trimmed.split_whitespace();
    let head = parts.next()?;
    ...
    // Find `exec` and look at what comes after `smellytype record`.
...
/// Niri's KDL `binds { Mod+Key { spawn "smellytype" "record" "ACTION"; } }`.
...
fn parse_niri_line(line: &str, source: &Path) -> Option<Binding> {
    ...
    if !trimmed.contains("smellytype") || !trimmed.contains("spawn") {
        return None;
    }
    // Form: `Mod+Key { spawn "smellytype" "record" "ACTION"; }`.
    ...
    if quoted.first().map(|s| s.as_str()) != Some("smellytype") {
        return None;
    }
...
fn action_from_command(cmd: &str) -> Option<String> {
    // Look for `smellytype <subcmd> <leaf>` in the command line.
    let lc = cmd.to_lowercase();
    let idx = lc.find("smellytype")?;
    let after = &cmd[idx + "smellytype".len()..];
...
mod tests {
    ...
    fn parses_hyprland_bindd() {
        let line = "bindd  = , HOME, SmellyType PTT (start), exec, smellytype record start";
    ...
    fn parses_hyprland_bindrd_with_mod() {
        let line = "bindrd = SUPER, F13, Stop, exec, smellytype record stop";
        let b = parse_hyprland_line(line, dummy_path()).unwrap();
    ...
    fn parses_hyprland_meeting_start() {
        let line = "bind = SUPER, M, Meeting start, exec, smellytype meeting start";
        let b = parse_hyprland_line(line, dummy_path()).unwrap();
    ...
    fn skips_hyprland_comments_and_unrelated() {
        assert!(parse_hyprland_line(
            "# bind = , HOME, ..., exec, smellytype record start",
            dummy_path()
        )
        .is_none());
    ...
    fn parses_sway_bindsym() {
        let line = "bindsym Mod4+Home exec smellytype record toggle";
    ...
    fn parses_sway_with_release_flag() {
        let line = "bindsym --release Mod4+Home exec smellytype record stop";
        let b = parse_sway_line(line, dummy_path()).unwrap();
        assert_eq!(b.keys, "Mod4+Home");
        assert_eq!(b.action, "record stop");
    }
    ...
    fn parses_niri_spawn() {
        let line = r#"    Mod+Home { spawn "smellytype" "record" "start"; }"#;
    ...
    fn parses_niri_meeting() {
        let line = r#"Mod+M { spawn "smellytype" "meeting" "start"; }"#;
        let b = parse_niri_line(line, dummy_path()).unwrap();
        assert_eq!(b.action, "meeting start");
    }
    ...
    fn rejects_unknown_action() {
        let line = "bindd = , HOME, ..., exec, smellytype record dance";
