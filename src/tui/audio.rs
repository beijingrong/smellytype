...
/// short-lived `smellytype configure --probe-audio-devices` subprocess because
    ...
    match state.field {
        Field::Device => {
            ...
            vec![
                ...
                Line::from(""),
                Line::from(
                    "Pick a specific device if you want smellytype to ignore the \
                     system default and stay locked to one mic — useful when \
                     you stream and don't want smellytype to grab your stream mic.",
                ),
            ]
        }
        Field::MaxDuration => vec![
            ...
            Line::from(
                "Safety cap. If you accidentally lock the PTT key down (or \
                 use toggle mode and forget), smellytype stops at this many \
        ...
        Field::FeedbackVolume => vec![
            ...
            Line::from(
                "Volume of the feedback cues, 0-100%. Independent of system \
                 volume — smellytype attenuates the sample at playback time.",
            ),
            Line::from(""),
            Line::from(
