...
pub struct AdvancedState {
    ...
    /// `d` that they want smellytype to fork the download in the background.
...
impl AdvancedState {
    ...
    pub fn save(&mut self) -> Action {
        ...
        // Hyprland/Sway/River, so the key-release event never reaches smellytype.
        ...
        match ed.save() {
            Ok(()) => {
                ...
                if let Some((from, to)) = auto_switched_model {
                    ...
                    // `smellytype setup --download --model <NAME>` directly.
...
fn guidance_for_field(state: &AdvancedState) -> Vec<Line<'_>> {
    match state.field {
        Field::GpuIsolation => vec![
            ...
            Line::from(
                "  • You see VRAM use creep upward over a long smellytype \
                 session.",
            ),
            Line::from(""),
            Line::from(Span::styled(
        ...
        Field::OnDemand => vec![
            ...
            Line::from(""),
            Line::from(
                "When on, smellytype loads the model only when recording starts \
        ...
        Field::Eager => vec![
            ...
            Line::from(""),
            Line::from(
                "SmellyType starts transcribing audio chunks while you're still \
        ...
        Field::GpuDevice => vec![
            ...
            Line::from(""),
            Line::from(
                "Picks which GPU smellytype targets on multi-GPU systems. The \
        ...
        Field::Streaming => vec![
            ...
            Line::from(""),
            Line::from(
                "SmellyType types text incrementally while you're still speaking \
            ...
            Line::from(Span::styled(
                "Requires a streaming-capable model (TDT v3 family with \
                 tokenizer.model). SmellyType prints a clear error at daemon \
...
/// transitions. The thread is fire-and-forget — smellytype's TUI doesn't
...
/// `x-canonical-private-synchronous:smellytype-model-download`, which tells
...
fn spawn_background_model_download(model_name: String) {
    std::thread::spawn(move || {
        const SYNC_HINT: &str = "string:x-canonical-private-synchronous:smellytype-model-download";
            ...
            Ok(()) => {
                let _ = std::process::Command::new("notify-send")
                    .args([
                        ...
                        "SmellyType",
                        &format!(
                            "Model download complete: {}. Restart smellytype to use it.",
                            model_name
                        ),
                    ])
                    .stdout(std::process::Stdio::null())
            ...
            Err(e) => {
                let _ = std::process::Command::new("notify-send")
                    .args([
                        ...
                        "SmellyType",
                        &format!(
                            "Model download failed: {}. Run `smellytype setup --download --model {}` to retry.",
                            e, model_name
                        ),
                    ])
                    .stdout(std::process::Stdio::null())
