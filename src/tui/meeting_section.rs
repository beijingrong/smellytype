...
fn guidance_for_field(state: &MeetingState) -> Vec<Line<'_>> {
    match state.field {
        Field::Enabled => vec![
            ...
            Line::from(""),
            Line::from(
                "Long-form recording mode. SmellyType chunks audio into \
                 segments, transcribes each, and stitches a continuous \
                 transcript with timestamps.",
            ),
            Line::from(""),
            Line::from(
                "Persists segments to ~/.local/share/smellytype/meetings/ so a \
        ...
        Field::AudioSource => vec![
            ...
            Line::from(
                "Mic + system loopback. SmellyType runs GTCRN echo cancellation \
