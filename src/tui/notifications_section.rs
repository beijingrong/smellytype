...
fn guidance_for_field(state: &NotificationsState) -> Vec<Line<'_>> {
    match state.field {
        Field::OnStart => vec![
            ...
            Line::from(""),
            Line::from(
                "Fires a desktop notification the moment smellytype begins \
                 capturing audio.",
            ),
            Line::from(""),
            Line::from(
        ...
        Field::OnStop => vec![
            ...
            Line::from(""),
            Line::from(
                "Notifies when smellytype stops recording and starts \
                 transcribing. Helpful when transcription takes a few \
                 seconds — you know smellytype heard the stop.",
            ),
        ],
        ...
        Field::ShowEngineIcon => vec![
            ...
            Line::from(
                "Helpful when you switch engines often or run multiple \
                 smellytype configurations side by side.",
            ),
        ],
    }
}
