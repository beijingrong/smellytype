//! Terminal UI for `smellytype configure`.
//!
//! Renders an interactive view over smellytype settings. The General section
...
// Exported beyond the crate so the binary's `smellytype config` handlers write
...
/// Entry point for the hidden `smellytype configure --probe-audio-devices`
...
pub fn run(
    ...
    if result? {
        ...
        println!("Restarted the smellytype daemon to apply saved config changes.");
    }
    Ok(())
}
...
fn dispatch_action(
    ...
    match action {
        ...
        Action::SwitchVariantAndDownload {
            ...
            // download writes to ~/.local/share/smellytype/models, no symlink
...
/// invoke whichever smellytype is on PATH because that's the user's installed
...
fn run_setup_model(engine: &str, model: &str) -> Result<(), String> {
    let _ = engine;
    let status = std::process::Command::new("smellytype")
        .args(["setup", "--download", "--model", model])
        .status()
        .map_err(|e| format!("could not invoke `smellytype setup --download`: {}", e))?;
    if status.success() {
        Ok(())
    } else {
        Err(format!(
            "smellytype setup --download exited with {}",
            status
        ))
    }
}

/// Best-effort `systemctl --user restart smellytype` after a binary swap or
...
fn restart_smellytype_daemon(app: &mut App) {
...
/// without a manual `systemctl --user restart smellytype`. The decision logic
...
fn render_help_overlay(f: &mut Frame) {
    ...
    let block = ratatui::widgets::Block::default()
        ...
        .title(" SmellyType Configuration — Help ");
    let inner = block.inner(rect);
    f.render_widget(block, rect);
    ...
    let lines = vec![
        ...
        Line::from("  D            Start or restart the smellytype daemon"),
...
fn render_title(f: &mut Frame, area: Rect) {
    ...
    let line = Line::from(vec![
        Span::raw(" SmellyType Configuration"),
