...
//! TUI. The OSD binaries (smellytype-osd, smellytype-osd-gtk4, smellytype-osd-native)
...
pub struct OsdState {
    ...
    /// True when neither smellytype-osd-gtk4 nor smellytype-osd-native is on
...
// list can still hand-edit ~/.config/smellytype/config.toml.
...
/// `smellytype-osd` launcher resolves the configured frontend by binary
/// name. Returns false when the user has the configure TUI installed
/// (typically via `smellytype-bin` or a source build) but doesn't have the
...
fn osd_binary_available() -> bool {
    ...
    const CANDIDATES: &[&str] = &[
        "/usr/bin/smellytype-osd-gtk4",
        "/usr/bin/smellytype-osd-native",
        "/usr/bin/smellytype-osd-quickshell",
        "/usr/lib/smellytype/smellytype-osd-gtk4",
        "/usr/lib/smellytype/smellytype-osd-native",
        "/usr/lib/smellytype/smellytype-osd-quickshell",
        "/usr/local/bin/smellytype-osd-gtk4",
        "/usr/local/bin/smellytype-osd-native",
        "/usr/local/bin/smellytype-osd-quickshell",
    ];
    ...
    if let Ok(path) = std::env::var("PATH") {
        for dir in path.split(':') {
            ...
            if Path::new(dir).join("smellytype-osd-gtk4").exists()
                || Path::new(dir).join("smellytype-osd-native").exists()
                || Path::new(dir).join("smellytype-osd-quickshell").exists()
            {
                return true;
            }
        }
...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    ...
    let missing_msg = "OSD binaries not installed. Saving the config will work but \
                       nothing will render until smellytype-osd-gtk4 (Arch: gtk4-layer-shell), \
                       smellytype-osd-native, or smellytype-osd-quickshell is installed.";
...
fn guidance_for_field(state: &OsdState) -> Vec<Line<'_>> {
    match state.field {
        Field::Enabled => vec![
            ...
            Line::from(
                "Master switch for the floating OSD. When off, both \
                 smellytype-osd-gtk4, smellytype-osd-native, and smellytype-osd-quickshell exit immediately at \
        ...
        Field::Frontend => vec![
            ...
            Line::from(""),
            Line::from(
                "Which OSD binary the smellytype-osd wrapper launches. Both \
        ...
        Field::Style => vec![
            ...
            Line::from(""),
            dim("Default: default. Package manifests live in smellytype-osd.toml."),
        ],
        Field::Palette => vec![
            heading("Palette"),
            Line::from(""),
        ...
        Field::TopMargin => vec![
            ...
            Line::from(
                "Vertical position of the OSD's top edge as a fraction of \
                 ...
                 so smellytype lands in the same band as the volume / brightness \
