...
impl EngineState {
    ...
    fn refresh_binary_match(&mut self) {
        ...
        if self.engine == "openvino" {
            if !inv.compiled_features.contains(&"openvino") {
                self.binary_switch_blocked = Some(
                    "OpenVINO isn't available in prebuilt smellytype binaries. \
        ...
        if inv.install_kind == InstallKind::Source {
            ...
            if !supported {
                self.binary_switch_blocked = Some(
                    "Source build: rebuild smellytype with the corresponding \
                ...
                None => {
                    self.binary_switch_blocked = Some(
                        "No installed binary supports this engine on this \
                         hardware. Install the matching smellytype variant first.",
                    );
                }
            }
        }
    ...
    pub fn save(&mut self) -> Action {
        ...
        // Whisper — always written; smellytype assumes a Whisper config exists.
    ...
    fn cycle(&mut self, delta: i32) {
        ...
        match field {
            FieldId::Engine => {
                ...
                // /usr/bin/smellytype can be symlinked to a binary that has the
...
///
/// We always check /usr/lib/smellytype/ first via `installed_engines(&inv)`,
/// regardless of whether the running TUI is a package binary or a source
/// build — the daemon runs whatever /usr/bin/smellytype resolves to, not
/// necessarily the same binary as the TUI. A developer running a source-built
/// `smellytype configure` on a host that also has the AUR package installed
...
fn installed_engine_choices() -> std::collections::HashSet<&'static str> {
    ...
    // Look at /usr/lib/smellytype/ first. enumerate_installed walks the dir
    ...
    // covers pure source builds with no /usr/lib/smellytype/. The
...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    let state = match &app.engine {
        ...
        None => {
            ...
            f.render_widget(
                Paragraph::new("Failed to load config; check ~/.config/smellytype/config.toml.")
                    .wrap(Wrap { trim: true }),
                inner,
            );
            return;
...
fn guidance(state: &EngineState) -> Vec<Line<'_>> {
    ...
    match state.current_field() {
        ...
        FieldId::WMode => vec![
            ...
            Line::from(
                "Shell out to a `whisper` CLI binary. Useful for testing \
                 custom builds without rebuilding smellytype.",
            ),
        ],
        ...
        FieldId::WThreads => vec![
            ...
            Line::from(""),
            Line::from(
                "Number of CPU threads Whisper uses. `auto` lets smellytype \
            ...
            let mut lines = vec![
                ...
                Line::from(
                    "Useful for proper nouns and technical terms. Examples: \
                     \"SmellyType, Hyprland, Claude.\" or \"Transcribe with proper \
        ...
        FieldId::PkModelType => vec![
            ...
            Line::from(
                "smellytype inspects the model directory and picks tdt or ctc \
        ...
        FieldId::WRemoteEndpoint => vec![
            ...
            Line::from(""),
            Line::from(
                "OpenAI-compatible Whisper API base URL. smellytype POSTs audio \
        ...
        FieldId::WRemoteApiKey => vec![
            ...
            Line::from(
                "If you'd rather not have it on disk, set the \
                 SMELLYTYPE_WHISPER_API_KEY environment variable instead and \
                 leave this unset.",
            ),
            Line::from(""),
            Line::from(Span::styled(
...
fn model_guidance(engine: &str, current: &str) -> Vec<Line<'static>> {
    ...
    let mut lines = vec![
        ...
        Line::from(""),
        Line::from(format!(
            "Inference checkpoint smellytype loads for {}. ←→ cycles through \
             the models smellytype knows about; pick whichever balances accuracy \
    ...
    lines.push(Line::from(Span::styled(
        "Models you haven't downloaded yet show up here too. Switch to one, \
         save, then run `smellytype setup model` to fetch the weights.",
        Style::default().fg(Color::Gray),
    )));
    if engine == "cohere" {
        lines.push(Line::from(""));
    ...
    vec![
        ...
        Line::from(format!(
            "Number of CPU threads ONNX Runtime uses for {} inference. \
             `auto` lets smellytype pick (typically physical-core count).",
            engine
        )),
        Line::from(""),
        Line::from(
