...
impl ConfigEditor {
    ...
    /// given, otherwise `~/.config/smellytype/config.toml`. Creates an empty
    ...
    /// is missing). Used by the CLI `smellytype config set` command, which has
    ...
    /// the value themselves — `smellytype config schema --json` reports both.
