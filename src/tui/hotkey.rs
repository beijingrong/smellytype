...
/// Sensible PTT key choices, in order. Values match what smellytype's evdev
...
fn guidance_enabled<'a>(state: &'a HotkeyState) -> Vec<Line<'a>> {
    let mut lines = vec![
        ...
        Line::from(""),
        Line::from(
            "When enabled, smellytype reads keyboard events directly from \
             /dev/input/event* (your user must be in the `input` group). It \
             owns the chosen PTT key globally — no compositor binding needed.",
        ),
        Line::from(""),
        Line::from(
            "When disabled, smellytype reads no keys. Bind your compositor (\
             Hyprland, Sway, Niri, KDE shortcuts) to call:",
        ),
        Line::from(Span::styled(
            "    smellytype record start    smellytype record stop",
            Style::default().fg(Color::Gray),
        )),
        Line::from(Span::styled(
            "    smellytype record toggle   smellytype record cancel",
            Style::default().fg(Color::Gray),
        )),
        Line::from(""),
    ];
    ...
    if !bindings.is_empty() {
        ...
        lines.push(Line::from(Span::styled(
            "No compositor bindings detected — smellytype will not receive any \
             PTT key events.",
    ...
    let mut lines = vec![
        ...
        Line::from(Span::styled(
            "(none) leaves cancellation off — kill the recording with \
             `smellytype record cancel` instead.",
            Style::default().fg(Color::Gray),
        )),
    ];
    if !state.enabled {
...
fn guidance_modifier<'a>(state: &'a HotkeyState) -> Vec<Line<'a>> {
    let mut lines = vec![
        ...
        Line::from(""),
        Line::from(
            "When this key is held alongside the PTT key, smellytype switches to \
