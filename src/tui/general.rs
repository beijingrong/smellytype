...
fn render_banner(f: &mut Frame, area: Rect, app: &App) {
    ...
    if app.restart_needed {
        lines.push(Line::from(Span::styled(
            "  Daemon restart required: systemctl --user restart smellytype",
...
fn render_matrix(f: &mut Frame, area: Rect, app: &App) {
    ...
    if app.inventory.install_kind == InstallKind::Source {
        let para = Paragraph::new(vec![
            ...
            Line::from("(/usr/lib/smellytype/smellytype-*). To enable a different"),
...
fn na_hint_lines<'a>(row: usize, col: usize) -> Vec<Line<'a>> {
    ...
    let suggestion = match (family, accel) {
        ...
        (Some(EngineFamily::Whisper), Some(Acceleration::Migraphx)) => {
            "For AMD GPU acceleration with Whisper, use Vulkan — smellytype's \
        ...
        Variant::OnnxCuda12 | Variant::OnnxCuda13 | Variant::OnnxCuda => VariantHint {
            ...
            body: &[
                "GPU inference via the CUDA execution provider. Best choice for \
                 ...
                 ONNX engine.",
                "smellytype ships separate cu12 and cu13 binaries; pick the one \
        ...
        Variant::OnnxMigraphx => VariantHint {
            ...
            body: &[
                "GPU inference for AMD GPUs via the MIGraphX execution provider \
                 (replaces the ROCm EP that was retired in smellytype 0.7.0).",
                "Note: this binary bundles an ONNX Runtime built with AVX-512, \
...
/// Run `systemctl --user start|restart smellytype` and report the outcome via
...
fn start_or_restart_daemon(app: &mut App) {
    ...
    let result = Command::new("systemctl")
        .args(["--user", action, "smellytype"])
        .output();

    let outcome = match result {
        Ok(out) if out.status.success() => SwitchOutcome {
            success: true,
            message: format!("smellytype daemon {}ed", action),
        },
        Ok(out) => {
            ...
            SwitchOutcome {
                success: false,
                message: format!("systemctl --user {} smellytype failed: {}", action, detail),
            }
        }
        Err(e) => SwitchOutcome {
            success: false,
