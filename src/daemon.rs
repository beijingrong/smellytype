...
async fn send_notification_with_lifetime(
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // keeps every SmellyType notification in a single slot. Posting from
...
/// Same mechanism as `smellytype record start --profile <name>`.
...
/// trigger files, partial writes from older smellytype versions, or a malicious
...
/// file-mode recording, and `smellytype record stop --wait` blocks on it.
...
pub struct Daemon {
    ...
    /// daemon shutdown) kill_on_drop's the spawned smellytype-osd process.
    ...
    dictation_started_at: Option<std::time::Instant>,
...
impl Daemon {
    ...
    fn restore_recording_media(&mut self) {
        crate::dictation_usage::finish(&mut self.dictation_started_at);
    ...
    async fn try_start_streaming(
        ...
        // Deferred-commit backends consume the override in the normal final
        // output pipeline. Reading it here would lose --clipboard/--file.
        let output_override = if self.config.engine == crate::config::TranscriptionEngine::Doubao {
            None
        } else {
            read_output_mode_override()
        };
    ...
    async fn stop_active_recording(
        ...
        if state.is_streaming() {
            ...
            if !file_output && self.config.engine != crate::config::TranscriptionEngine::Doubao {
    ...
    async fn stop_streaming_capture(&mut self, audio_capture: &mut Option<Box<dyn AudioCapture>>) {
        if self.config.engine == crate::config::TranscriptionEngine::Doubao {
            // Stop the microphone first, then drain queued chunks and the
            // resampler tail. Aborting the pump here clips the last syllable.
            if let Some(mut c) = audio_capture.take() {
                let _ = c.stop().await;
            }
            if let Some(mut pump) = self.level_emitter_task.take() {
                if tokio::time::timeout(std::time::Duration::from_secs(2), &mut pump)
                    .await
                    .is_err()
                {
                    pump.abort();
    ...
    {
        let mut audio_config = self.config.audio.clone();
        if self.config.engine == crate::config::TranscriptionEngine::Doubao {
            audio_config.sample_rate = 16000;
        }
        match audio::create_capture(&audio_config) {
            Ok(mut capture) => match capture.start().await {
                Ok(chunk_rx) => {
                    ...
                    let (streaming_tx, streaming_rx) =
                        tokio::sync::mpsc::channel::<Vec<f32>>(capacity);
                    ...
                    self.level_emitter_task = Some(handle);
                    if self.config.engine == crate::config::TranscriptionEngine::Doubao {
                        self.dictation_started_at = Some(crate::dictation_usage::begin());
                    }
    ...
    async fn get_transcriber_for_recording(
        ...
        if self.config.on_demand_loading() {
            ...
            match self.config.engine {
                ...
                | crate::config::TranscriptionEngine::Doubao => {
                        ...
                        match audio::DualCapture::new(&meeting_audio_config, loopback_device) {
                            Ok(mut capture) => {
                                if let Err(e) = capture.start().await {
                                    ...
                                    return Err(crate::error::SmellyTypeError::Audio(e));
                                }
    ...
    fn warn_on_variant_mismatch(&self) {
        ...
        let title = format!("SmellyType: {} unavailable", mismatch.configured_engine);
        let body = match &mismatch.remediation {
            crate::setup::variant_check::Remediation::SwitchToVariant { target } => format!(
                "{} was built without --features {}. \
                 Run `sudo smellytype setup onnx --enable` (or open `smellytype configure` and press F2) \
                 to switch to {}.",
                active,
            ...
            crate::setup::variant_check::Remediation::Rebuild { feature } => format!(
                "This source build was compiled without --features {}. \
                 Rebuild smellytype with that feature to enable the {} engine.",
    ...
    pub async fn run(&mut self) -> Result<()> {
        tracing::info!("Starting smellytype daemon");
        ...
        let mut sigusr1 = signal(SignalKind::user_defined1()).map_err(|e| {
            crate::error::SmellyTypeError::Config(format!(
        ...
        let mut sigusr2 = signal(SignalKind::user_defined2()).map_err(|e| {
            crate::error::SmellyTypeError::Config(format!(
                "Failed to set up SIGUSR2 handler: {}",
                e
            ))
        })?;
        ...
        Config::ensure_directories().map_err(|e| {
            crate::error::SmellyTypeError::Config(format!("Failed to create directories: {}", e))
        })?;
        ...
        let lock_path = Config::runtime_dir().join("smellytype.lock");
            ...
            Err(_) => {
                ...
                if cleanup_stale_lockfile(&lock_path) {
                    ...
                    if let Err(e) = pidlock.acquire() {
                        ...
                        return Err(crate::error::SmellyTypeError::Config(format!(
                            "Another smellytype instance is already running (lock error: {:?})",
                            e
                        )));
                    }
                    ...
                    tracing::error!(
                        "Failed to acquire lock: another smellytype instance is already running"
                    );
                    return Err(crate::error::SmellyTypeError::Config(
                        "Another smellytype instance is already running".to_string(),
                    ));
                }
                #[cfg(not(unix))]
                {
            ...
            .enabled
        {
            ...
            tracing::info!(
                "Built-in hotkey disabled, use 'smellytype record' commands or compositor keybindings"
            );
            None
        };
            ...
            .enabled
        {
            ...
            match hotkey::create_listener(&self.config.hotkey, secondary_model) {
                ...
                Err(e) => {
                    tracing::warn!("Failed to create hotkey listener: {}. Use 'smellytype record' commands instead.", e);
                    None
                }
            }
        } else {
        ...
        let hotkey_listener: Option<()> = {
            if self.config.hotkey.enabled {
                tracing::warn!(
                    "Built-in hotkey not supported on this platform, use 'smellytype record' commands"
                );
            }
            None
        };
                ...
                crate::config::TranscriptionEngine::Whisper => {
                    if self.config.whisper.streaming {
                        ...
                        if let Err(e) = model_manager.preload_primary() {
                            ...
                            return Err(crate::error::SmellyTypeError::Transcribe(e));
                        }
                    }
                }
                crate::config::TranscriptionEngine::Parakeet
        ...
        let mut hotkey_rx = if let Some(ref mut listener) = hotkey_listener {
            match listener.start() {
                Ok(rx) => Some(rx),
                Err(e) => {
                    tracing::warn!("Failed to start hotkey listener: {}. Use 'smellytype record' commands instead.", e);
                    None
                }
            }
        } else {
        ...
        loop {
            tokio::select! {
                ...
                _ = tokio::time::sleep(Duration::from_millis(100)), if state.is_recording() => {
                    ...
                    if timeout_fired {
                        // A hard-capped session is ending on smellytype's own
                ...
                _ = sigusr1.recv() => {
                    ...
                    if state.is_idle() {
                        // Read model override from file (set by `smellytype record start --model X`)
                    ...
                    match event {
                        Some(StreamingEvent::Preview { text }) => {
                            if self.config.osd.enabled {
                                crate::osd::preview::publish(&text);
                            }
                        }
...
mod tests {
    ...
    fn test_pidlock_acquisition_succeeds() {
        with_test_runtime_dir(|dir| {
            let lock_path = dir.join("smellytype.lock");
            let lock_path_str = lock_path.to_string_lossy().to_string();
