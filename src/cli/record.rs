...
pub enum RecordAction {
    ...
    Start {
        ...
        /// tools that draw their own dictation UI and do not want smellytype's
...
mod tests {
    ...
    fn test_record_cancel() {
        let cli = Cli::parse_from(["smellytype", "record", "cancel"]);
        match cli.command {
    ...
    fn test_record_start_no_override() {
        let cli = Cli::parse_from(["smellytype", "record", "start"]);
        match cli.command {
    ...
    fn test_record_start_paste_override() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--paste"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
    ...
    fn test_record_start_clipboard_override() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--clipboard"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn test_record_start_type_override() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--type"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn stop_defaults_to_not_waiting() {
        let cli = Cli::parse_from(["smellytype", "record", "stop"]);
        match cli.command {
            Some(Commands::Record {
                action:
                    RecordAction::Stop {
    ...
    fn waiting_flags_require_wait() {
        ...
        for extra in [
            ...
            let mut argv = vec!["smellytype", "record", "stop"];
    ...
    fn stop_wait_still_takes_an_output_mode() {
        let cli = Cli::parse_from(["smellytype", "record", "stop", "--wait", "--paste"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn test_record_stop_paste_override() {
        let cli = Cli::parse_from(["smellytype", "record", "stop", "--paste"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn test_record_toggle_paste_override() {
        let cli = Cli::parse_from(["smellytype", "record", "toggle", "--paste"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn test_record_start_file_with_path() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--file=out.txt"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn test_record_start_model_override() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--model", "large-v3-turbo"]);
        match cli.command {
            Some(Commands::Record { action }) => {
    ...
    fn test_record_start_file_without_path() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--file"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn test_record_start_file_with_absolute_path() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--file=/tmp/output.txt"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn test_record_toggle_model_override() {
        let cli = Cli::parse_from(["smellytype", "record", "toggle", "--model", "medium.en"]);
        match cli.command {
            Some(Commands::Record { action }) => {
    ...
    fn test_record_toggle_file_with_path() {
        let cli = Cli::parse_from(["smellytype", "record", "toggle", "--file=out.txt"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn test_record_toggle_file_without_path() {
        let cli = Cli::parse_from(["smellytype", "record", "toggle", "--file"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(
                    action.output_mode_override(),
    ...
    fn test_record_start_file_mutually_exclusive_with_paste() {
        let result =
            Cli::try_parse_from(["smellytype", "record", "start", "--file=out.txt", "--paste"]);
    ...
    fn test_record_start_file_mutually_exclusive_with_type() {
        let result =
            Cli::try_parse_from(["smellytype", "record", "start", "--file=out.txt", "--type"]);
    ...
    fn test_record_start_with_profile() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--profile", "slack"]);
        match cli.command {
            Some(Commands::Record { action }) => {
    ...
    fn test_record_toggle_with_profile() {
        let cli = Cli::parse_from(["smellytype", "record", "toggle", "--profile", "code"]);
        match cli.command {
            Some(Commands::Record { action }) => {
    ...
    fn test_record_start_auto_submit() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--auto-submit"]);
        match cli.command {
            Some(Commands::Record { action }) => {
    ...
    fn test_record_no_osd_flag() {
        for sub in ["start", "toggle"] {
            let cli = Cli::parse_from(["smellytype", "record", sub, "--no-osd"]);
            match cli.command {
                Some(Commands::Record { action }) => {
    ...
    fn test_record_no_osd_defaults_off() {
        for args in [
            vec!["smellytype", "record", "start"],
            vec!["smellytype", "record", "toggle"],
            vec!["smellytype", "record", "stop"],
            vec!["smellytype", "record", "cancel"],
        ] {
    ...
    fn test_record_start_no_auto_submit() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--no-auto-submit"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(action.auto_submit_override(), Some(false));
            }
    ...
    fn test_record_toggle_auto_submit() {
        let cli = Cli::parse_from(["smellytype", "record", "toggle", "--auto-submit"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(action.auto_submit_override(), Some(true));
            }
    ...
    fn test_record_start_shift_enter_newlines() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--shift-enter-newlines"]);
        match cli.command {
            Some(Commands::Record { action }) => {
    ...
    fn test_record_start_no_shift_enter_newlines() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--no-shift-enter-newlines"]);
        match cli.command {
            Some(Commands::Record { action }) => {
    ...
    fn test_record_start_smart_auto_submit_enable() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--smart-auto-submit"]);
        match cli.command {
            Some(Commands::Record { action }) => {
    ...
    fn test_record_start_no_smart_auto_submit() {
        let cli = Cli::parse_from(["smellytype", "record", "start", "--no-smart-auto-submit"]);
        match cli.command {
            Some(Commands::Record { action }) => {
    ...
    fn test_record_toggle_smart_auto_submit_enable() {
        let cli = Cli::parse_from(["smellytype", "record", "toggle", "--smart-auto-submit"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(action.smart_auto_submit_override(), Some(true));
            }
    ...
    fn test_record_toggle_no_smart_auto_submit() {
        let cli = Cli::parse_from(["smellytype", "record", "toggle", "--no-smart-auto-submit"]);
        match cli.command {
            Some(Commands::Record { action }) => {
                assert_eq!(action.smart_auto_submit_override(), Some(false));
            }
