...
#[derive(Parser)]
#[command(name = "smellytype")]
#[command(author, version, about = "Cloud dictation for Omarchy / Wayland")]
#[command(long_about = "\
SmellyType is a push-to-talk voice-to-text tool for Linux.\n\
Optimized for Wayland, works on X11 too.")]
#[command(after_help = "\
QUICK START:
  smellytype                     Start daemon with hotkey detection
  smellytype record toggle       Toggle recording (for compositor keybindings)
  smellytype configure           Open cloud dictation settings
  smellytype record cancel       Cancel recording
  smellytype status --follow     Watch daemon status (Waybar integration)

See 'smellytype --help' for all options or 'man smellytype' for full docs.")]
#[command(after_long_help = "\
QUICK START:
  smellytype                     Start daemon with hotkey detection
  smellytype daemon              Same as above (explicit)
  smellytype record toggle       Toggle recording (for compositor keybindings)
  smellytype record start        Start recording
  smellytype record stop         Stop recording and transcribe
  smellytype record cancel       Cancel current recording
  smellytype status              Show daemon status
  smellytype setup               Check dependencies and download models
  smellytype config              Show current configuration

EXAMPLES:
  smellytype configure           Open cloud dictation settings (Whisper, Parakeet, or Moonshine)
  smellytype setup waybar        Show Waybar integration config
  smellytype record cancel       Cancel recording (Vulkan/CUDA/MIGraphX)
  smellytype setup onnx          Switch between Whisper and ONNX engines
  smellytype status --follow --format json   Waybar integration

See 'smellytype <command> --help' for more info on a command.
See 'man smellytype' or docs/INSTALL.md for setup instructions.")]
pub struct Cli {
    /// Path to config file
    ...
    /// API key for remote server (or use SMELLYTYPE_WHISPER_API_KEY env var)
    #[arg(
        long,
        value_name = "KEY",
        help_heading = "Whisper",
    ...
    /// Doubao ASR API key (prefer DOUBAO_API_KEY environment variable)
    #[arg(
        long,
        value_name = "KEY",
        help_heading = "Doubao",
    ...
    /// Doubao streaming resource (default: volc.seedasr.sauc.duration)
    #[arg(
        long,
        value_name = "ID",
        help_heading = "Doubao",
    ...
    /// Maximum wait for Doubao's final result after release (seconds, default 60)
    #[arg(long, value_name = "SECONDS", help_heading = "Doubao", hide_short_help = true,
        value_parser = clap::value_parser!(u64).range(1..=300))]
    pub doubao_final_timeout_secs: Option<u64>,
...
mod tests {
    ...
    fn test_engine_flag_with_model_flag() {
        let cli = Cli::parse_from(["smellytype", "--engine", "whisper", "--model", "large-v3"]);
        assert_eq!(cli.engine, Some("whisper".to_string()));
    ...
    #[test]
    fn test_driver_flag() {
        let cli = Cli::parse_from(["smellytype", "--driver=ydotool,wtype"]);
        assert_eq!(cli.driver, Some("ydotool,wtype".to_string()));
    }

    #[test]
    fn test_driver_flag_single() {
        let cli = Cli::parse_from(["smellytype", "--driver=ydotool"]);
    ...
    fn test_transcribe_engine_flag_not_set() {
        let cli = Cli::parse_from(["smellytype", "transcribe", "test.wav"]);
        match cli.command {
