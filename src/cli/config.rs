//! `smellytype config` subcommand actions.
...
pub enum ConfigAction {
    ...
    // not accept. `smellytype info engines` reports what this binary can
    // actually switch to.
    #[command(long_about = "\
        Set a single configuration value in the on-disk config file\n\n\
        KEY is a dotted TOML path from the allowlist printed by \
        `smellytype config schema`; VALUE is type-checked and range-checked \
        ...
        Examples:\n  \
        smellytype config set engine whisper\n  \
        smellytype config set hotkey.key F13\n  \
        smellytype config set audio.feedback.volume 0.6\n  \
        smellytype config set text.replacements.btw 'by the way'\n\n\
        See `smellytype info engines` for the engines this binary supports.")]
    Set {
    ...
    #[command(long_about = "\
        ...
        Examples:\n  \
        smellytype config unset whisper.initial_prompt\n  \
        smellytype config unset text.replacements.btw")]
    Unset {
    ...
    #[command(long_about = "\
        ...
        Examples:\n  \
        smellytype config get hotkey.mode\n  \
        smellytype config get audio.device --json\n  \
        smellytype config get --json")]
    Get {
