//! `smellytype setup` subcommand actions and compositor variants.
...
pub enum SetupAction {
    ...
    /// Install smellytype as a systemd user service (Linux)
    Systemd {
    ...
    /// Install smellytype as a LaunchAgent (macOS)
    ...
    /// Install SmellyType.app bundle with Login Items (macOS, recommended)
    /// Creates /Applications/SmellyType.app and adds to Login Items.
    ...
    /// Switch the active binary variant (used by `smellytype configure` via pkexec)
    #[command(hide = true)]
    Variant {
        /// Variant binary name (e.g., smellytype-avx512, smellytype-onnx-cuda)
        #[arg(long, value_name = "NAME")]
        to: String,
    },
    ...
    /// Install the Quickshell QML tree for the smellytype-osd-quickshell launcher
    ///
    /// Copies shell.qml, OsdSurface.qml, EnginePicker.qml,
    /// MeetingControls.qml, and the smellytype-shared module into
    /// $XDG_DATA_HOME/smellytype/quickshell/ (or ~/.local/share/smellytype/quickshell/
    ...
    Quickshell {
        ...
        ///
        /// Search order: $SMELLYTYPE_QUICKSHELL_SOURCE_DIR,
        /// <binary>/../share/smellytype/quickshell/, /usr/share/smellytype/quickshell/,
        /// ./quickshell/
        #[arg(long, value_name = "DIR")]
        ...
        /// Override the source path of the smellytype-audio-bridge binary.
        ///
        /// Search order (when omitted): $SMELLYTYPE_AUDIO_BRIDGE_BINARY,
        /// <binary>/../lib/smellytype/smellytype-audio-bridge,
        /// /usr/lib/smellytype/smellytype-audio-bridge, `which smellytype-audio-bridge`,
        /// target/release/smellytype-audio-bridge, target/debug/smellytype-audio-bridge.
        #[arg(long, value_name = "PATH")]
        bridge: Option<std::path::PathBuf>,

        /// Override the symlink location for smellytype-audio-bridge.
        ///
        /// Defaults to $XDG_BIN_HOME/smellytype-audio-bridge or
        /// ~/.local/bin/smellytype-audio-bridge. Must live under the user's
        /// $HOME unless you also pass --force.
        #[arg(long, value_name = "PATH")]
        bridge_target: Option<std::path::PathBuf>,

        /// Skip installing the smellytype-audio-bridge symlink.
        ///
...
mod tests {
    ...
    fn test_setup_quiet_flag() {
        let cli = Cli::parse_from(["smellytype", "setup", "--quiet"]);
        match cli.command {
    ...
    fn test_setup_progress_format() {
        ...
        let cli = Cli::parse_from(["smellytype", "setup"]);
        match cli.command {
            Some(Commands::Setup {
                progress_format, ..
            }) => assert_eq!(
        ...
        assert!(
            Cli::try_parse_from(["smellytype", "setup", "--progress-format", "ndjson"]).is_err(),
    ...
    fn test_setup_no_post_install_flag() {
        let cli = Cli::parse_from(["smellytype", "setup", "--no-post-install"]);
        match cli.command {
            Some(Commands::Setup {
                no_post_install,
                quiet,
    ...
    fn test_setup_quiet_with_download() {
        let cli = Cli::parse_from(["smellytype", "setup", "--quiet", "--download"]);
        match cli.command {
            Some(Commands::Setup {
                quiet, download, ..
            }) => {
    ...
    fn test_setup_both_quiet_flags() {
        ...
        let cli = Cli::parse_from(["smellytype", "setup", "--quiet", "--no-post-install"]);
        match cli.command {
            Some(Commands::Setup {
                quiet,
                no_post_install,
    ...
    fn test_setup_no_post_install_with_download() {
        let cli = Cli::parse_from(["smellytype", "setup", "--no-post-install", "--download"]);
        match cli.command {
            Some(Commands::Setup {
                quiet,
                no_post_install,
    ...
    fn test_setup_model_without_download() {
        ...
        let cli = Cli::parse_from(["smellytype", "setup", "--model", "small.en"]);
        match cli.command {
            Some(Commands::Setup {
                download, model, ..
            }) => {
    ...
    fn test_setup_dms_install() {
        let cli = Cli::parse_from(["smellytype", "setup", "dms", "--install"]);
        match cli.command {
            Some(Commands::Setup {
                action:
    ...
    fn test_setup_dms_uninstall() {
        let cli = Cli::parse_from(["smellytype", "setup", "dms", "--uninstall"]);
        match cli.command {
            Some(Commands::Setup {
                action:
                    Some(SetupAction::Dms {
    ...
    fn test_setup_dms_qml() {
        let cli = Cli::parse_from(["smellytype", "setup", "dms", "--qml"]);
        match cli.command {
            Some(Commands::Setup {
                action:
                    Some(SetupAction::Dms {
    ...
    fn test_setup_dms_default() {
        let cli = Cli::parse_from(["smellytype", "setup", "dms"]);
        match cli.command {
            Some(Commands::Setup {
                action:
                    Some(SetupAction::Dms {
