...
pub enum MeetingAction {
    ...
    Start {
        ...
        #[arg(
            ...
            env = "SMELLYTYPE_MEETING_DIARIZATION",
        )]
...
mod tests {
    ...
    fn test_meeting_start_diarization_simple_flag() {
        let cli = Cli::parse_from(["smellytype", "meeting", "start", "--diarization", "simple"]);
        match cli.command {
    ...
    fn test_meeting_start_diarization_rejects_invalid() {
        let result =
            Cli::try_parse_from(["smellytype", "meeting", "start", "--diarization", "bogus"]);
        assert!(
            result.is_err(),
    ...
    /// single test to avoid `SMELLYTYPE_MEETING_DIARIZATION` leaking between
    ...
    fn test_meeting_start_diarization_env_and_default() {
        ...
        std::env::remove_var("SMELLYTYPE_MEETING_DIARIZATION");

        // No flag, no env var → no override.
        let cli = Cli::parse_from(["smellytype", "meeting", "start"]);
        match cli.command {
            Some(Commands::Meeting {
                action: MeetingAction::Start { diarization, title },
            }) => {
        ...
        std::env::set_var("SMELLYTYPE_MEETING_DIARIZATION", "ml");
        let cli = Cli::parse_from(["smellytype", "meeting", "start"]);
        std::env::remove_var("SMELLYTYPE_MEETING_DIARIZATION");
        match cli.command {
            Some(Commands::Meeting {
