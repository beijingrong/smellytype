...
//!
//! The shape emitted by `format_state_json` is the API smellytype exposes to
//! every downstream status consumer — Waybar via `smellytype status --follow`,
...
/// waybar tooltips and `smellytype info` output. Whisper variants get a "CPU"/"GPU"
...
pub fn format_state_json(
    ...
    let (text, base_tooltip) = match state {
        ...
        "idle" => (&icons.idle, "SmellyType ready - hold hotkey to record"),
        "stopped" => (&icons.stopped, "SmellyType not running"),
...
mod tests {
    ...
    /// wins no matter where /usr/bin/smellytype points. This covers both the
    ...
    fn daemon_variant_settles_the_backend_label() {
        let label = daemon_backend_label(
            ...
            Some(Path::new("/usr/lib/smellytype/smellytype-avx2")),
            Path::new("/usr/lib/smellytype/cuda-13/smellytype-onnx-cuda-13"),
        );
    ...
    fn foreign_daemon_binary_reports_custom() {
        let label = daemon_backend_label(
            None,
            Some(Path::new(
                "/home/user/.local/opt/smellytype-custom/libexec/smellytype",
            )),
            Path::new("/usr/lib/smellytype/cuda-13/smellytype-onnx-cuda-13"),
        );
    ...
    fn daemon_running_the_cli_binary_defers_to_install_detection() {
        let exe = Path::new("/home/user/smellytype/target/release/smellytype");
    ...
    fn no_daemon_defers_to_install_detection() {
        let cli = Path::new("/usr/lib/smellytype/smellytype-avx2");
        assert_eq!(daemon_backend_label(None, None, cli), None);
    }

    /// Regression: after `smellytype record cancel` the daemon writes "idle"
    ...
    fn format_state_json_pins_byte_exact_output() {
        ...
        assert_eq!(
            ...
            r#"{"text": "I", "alt": "idle", "class": "idle", "tooltip": "SmellyType ready - hold hotkey to record"}"#,
        );
        assert_eq!(
            format_state_json("stopped", &icons, None),
            r#"{"text": "X", "alt": "stopped", "class": "stopped", "tooltip": "SmellyType not running"}"#,
        );
