//! `smellytype-osd-gtk4` — GTK4 + gtk4-layer-shell on-screen mic visualizer
//! for the SmellyType daemon.
//!
...
//! the daemon's audio Unix socket via [`smellytype::osd::ipc::run_ipc_loop`],
...
use smellytype::audio::levels::{AudioFrame, FRAME_HZ};
use smellytype::config::Config as SmellyTypeConfig;
use smellytype::osd::config::{OsdConfig, OsdPosition};
use smellytype::osd::ipc::{resolve_socket_path, run_ipc_loop, FrameRing, DEFAULT_RING_DEPTH};
use smellytype::osd::theme::ThemeWatcher;
use smellytype::osd::visual::{
    peak_meter_fraction, project_envelope, MeterZone, Palette, PeakHold,
};

/// Load the `[osd]` section from the smellytype config file, falling back to
...
fn load_osd_config_from_file(explicit: Option<&std::path::Path>) -> OsdConfig {
    let path = explicit
        ...
        .or_else(SmellyTypeConfig::default_path);
...
const APP_ID: &str = "io.smellytype.OsdGtk4";
...
/// (`smellytype record start --no-osd`).
///
...
fn osd_suppressed() -> bool {
    ...
    std::path::Path::new(&base)
        .join("smellytype/osd_suppressed")
        .exists()
}
...
#[command(
    name = "smellytype-osd-gtk4",
    version,
    about = "SmellyType on-screen mic visualizer (GTK4 + gtk4-layer-shell)"
)]
struct Args {
    /// Path to the smellytype config file. Defaults to
    /// `~/.config/smellytype/config.toml`. Only the `[osd]` section is read.
    #[arg(long, env = "SMELLYTYPE_CONFIG")]
    config: Option<PathBuf>,

    /// Path to the audio-frame Unix socket. Defaults to
    /// `$XDG_RUNTIME_DIR/smellytype/audio.sock`.
    #[arg(long, env = "SMELLYTYPE_OSD_SOCKET")]
    socket: Option<PathBuf>,

    /// Seconds to wait between reconnect attempts when the daemon is down.
    #[arg(long, default_value = "1.0", env = "SMELLYTYPE_OSD_RECONNECT_SECS")]
    reconnect_secs: f32,

    /// Print one debug line per N frames received (0 = quiet).
    #[arg(long, default_value = "0", env = "SMELLYTYPE_OSD_LOG_EVERY")]
    log_every: u32,

    /// Held-peak decay rate in dB/sec.
    #[arg(long, default_value = "6.0", env = "SMELLYTYPE_OSD_PEAK_DECAY")]
    peak_decay_db_per_sec: f32,

    /// Surface width in physical pixels.
    #[arg(long, env = "SMELLYTYPE_OSD_WIDTH")]
    width_px: Option<u32>,

    /// Surface height in physical pixels.
    #[arg(long, env = "SMELLYTYPE_OSD_HEIGHT")]
    height_px: Option<u32>,

    /// Margin from the screen edge in physical pixels.
    #[arg(long, env = "SMELLYTYPE_OSD_MARGIN")]
    ...
    #[arg(long, env = "SMELLYTYPE_OSD_GAIN")]
...
fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(
        "smellytype-osd-gtk4 starting; socket={:?} size={}x{} margin={} pos={:?}",
        socket_path,
        osd_cfg.width_px,
        osd_cfg.height_px,
        osd_cfg.margin_px,
...
fn build_window(app: &Application, cfg: &OsdConfig, palette: Palette, state: Arc<SharedState>) {
    ...
    window.set_namespace(Some("smellytype-osd"));
    ...
    // already see for volume/brightness/media-key feedback so the smellytype
