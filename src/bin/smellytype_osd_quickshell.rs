//! `smellytype-osd-quickshell` — a tiny launcher that finds smellytype's Quickshell
...
//! sibling QML imports (`import "smellytype-shared" as VT`) resolve through
...
//! the launcher from a hotkey, a short-lived shell, or `smellytype setup
...
//! `smellytype-osd` (the dispatcher) via `tokio::process::Command` with
...
//! supervisor sets `SMELLYTYPE_OSD_SUPERVISED=1` and the dispatcher then
...
//! 2. `SMELLYTYPE_OSD_QML_PATH` env var (same accept-either rules)
//! 3. `$XDG_DATA_HOME/smellytype/quickshell/`
//! 4. `~/.local/share/smellytype/quickshell/`
//! 5. `/usr/share/smellytype/quickshell/`
...
use smellytype::config;
use smellytype::osd::style;

const QS_BIN: &str = "qs";
const SHELL_FILE: &str = "shell.qml";
const SHELL_SUBDIR: &str = "smellytype/quickshell";
...
fn main() -> ExitCode {
    ...
    if raw_args.iter().any(|a| a == "--version" || a == "-V") {
        println!("smellytype-osd-quickshell {}", env!("CARGO_PKG_VERSION"));
        return ExitCode::SUCCESS;
    }
    ...
    let shell_dir = match resolve_shell_dir(cli_qml_path) {
        ...
        None => {
            eprintln!(
                "smellytype-osd-quickshell: could not find '{SHELL_FILE}' for the Quickshell OSD.\n\
                 \n\
                 Searched:\n    \
                     --qml-path <PATH>\n    \
                     $SMELLYTYPE_OSD_QML_PATH\n    \
                     $XDG_DATA_HOME/{SHELL_SUBDIR}/\n    \
                     ~/.local/share/{SHELL_SUBDIR}/\n    \
                     /usr/share/{SHELL_SUBDIR}/\n    \
                     ./quickshell/\n\
                 \n\
                 Install smellytype's Quickshell files (e.g. `smellytype setup quickshell`)\n\
    ...
    if which::which(QS_BIN).is_err() {
        eprintln!(
            "smellytype-osd-quickshell: '{QS_BIN}' (Quickshell) is not installed on PATH.\n\
             \n\
             ...
             Or switch to a different OSD frontend:\n    \
                 smellytype config set osd.frontend gtk4"
        );
    ...
    let runtime_style = match style::resolve_runtime_style(&config.osd, style_override) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("smellytype-osd-quickshell: failed to resolve OSD style: {e}");
            return ExitCode::from(3);
        }
    };
    let style_file = match style::write_runtime_style(&runtime_style) {
        Ok(p) => p,
        Err(e) => {
            eprintln!("smellytype-osd-quickshell: failed to write OSD style runtime file: {e}");
            return ExitCode::from(3);
        }
    };
    ...
    cmd.env("SMELLYTYPE_OSD_STYLE_FILE", &style_file);
    cmd.arg("-p").arg(&shell_dir).args(&rest);
    let err = cmd.exec();
    eprintln!(
        "smellytype-osd-quickshell: failed to exec '{QS_BIN}' with shell dir '{}': {err}",
        shell_dir.display()
    );
    ExitCode::from(1)
}
...
fn print_help() {
    println!(
        "smellytype-osd-quickshell {} — launcher for the Quickshell-based smellytype OSD\n\
         \n\
         USAGE:\n    \
             smellytype-osd-quickshell [--qml-path PATH] [QUICKSHELL ARGS...]\n\
         \n\
         OPTIONS:\n    \
             ...
             --config <FILE>      Read smellytype config from FILE.\n    \
         ...
         \n\
         ENV:\n    \
             SMELLYTYPE_OSD_QML_PATH   Same as --qml-path.\n    \
             SMELLYTYPE_OSD_STYLE      Same as --style.\n    \
             SMELLYTYPE_CONFIG         Path to smellytype config.toml.\n",
...
fn parse_args(
    ...
    let mut config: Option<PathBuf> = env::var("SMELLYTYPE_CONFIG").ok().map(PathBuf::from);
...
fn resolve_shell_dir(cli: Option<PathBuf>) -> Option<PathBuf> {
    ...
    if let Ok(env_path) = env::var("SMELLYTYPE_OSD_QML_PATH") {
    ...
    // Development convenience: running `cargo run --bin smellytype-osd-quickshell`
