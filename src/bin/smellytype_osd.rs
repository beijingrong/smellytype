//! `smellytype-osd` — a tiny launcher that picks between the `smellytype-osd-gtk4`,
//! `smellytype-osd-native`, and `smellytype-osd-quickshell` frontends and execs
//! the chosen one.
//!
//! The user's preference comes from (in priority order):
//!
//! 1. `--frontend gtk4|native|quickshell` on the command line
//! 2. `SMELLYTYPE_OSD_FRONTEND=gtk4|native|quickshell` env var
//! 3. `[osd] frontend = "gtk4|native|quickshell"` in `~/.config/smellytype/config.toml`
//! 4. Default: `gtk4`
//!
...
//! `osd-quickshell` thus get a working `smellytype-osd` regardless of config
...
//!
//! `smellytype-osd-quickshell` passes `-d` to `qs` by default so that
...
//!
//! The supervisor sets `SMELLYTYPE_OSD_SUPERVISED=1` on the dispatcher's
...
use smellytype::osd::config::{OsdConfig, OsdFrontend};

const NATIVE_BIN: &str = "smellytype-osd-native";
const GTK4_BIN: &str = "smellytype-osd-gtk4";
const QUICKSHELL_BIN: &str = "smellytype-osd-quickshell";
...
fn main() -> ExitCode {
    ...
    if raw_args.iter().any(|a| a == "--version" || a == "-V") {
        println!("smellytype-osd {}", env!("CARGO_PKG_VERSION"));
        return ExitCode::SUCCESS;
    }
    ...
    let preferred = cli_frontend
        .or_else(|| {
            env::var("SMELLYTYPE_OSD_FRONTEND")
                .ok()
    ...
    let chosen = match resolve_installed(preferred) {
        ...
        None => {
            eprintln!(
                "smellytype-osd: none of '{GTK4_BIN}', '{NATIVE_BIN}', or '{QUICKSHELL_BIN}' \
                 ...
                 If you built from source, enable at least one OSD feature:\n\
                   ...
                   cargo build --release --bin smellytype-osd-quickshell  # Quickshell launcher\n\
                 \n\
    ...
    let supervised = env::var("SMELLYTYPE_OSD_SUPERVISED")
        .map(|v| v == "1")
        .unwrap_or(false);
    ...
    eprintln!(
        "smellytype-osd: failed to exec '{}': {err}",
        chosen.path.display()
    );
    ExitCode::from(1)
}
...
fn print_help() {
    println!(
        "smellytype-osd {} — launcher for the on-screen mic visualizer\n\
         \n\
         USAGE:\n    \
             smellytype-osd [--frontend gtk4|native|quickshell] [FRONTEND ARGS...]\n\
         \n\
         OPTIONS:\n    \
         ...
         \n\
         ENV:\n    \
             SMELLYTYPE_OSD_FRONTEND   Same as --frontend.\n    \
             SMELLYTYPE_CONFIG         Path to the smellytype config file.\n",
...
/// Load the `[osd] frontend` value from the smellytype config file, falling
...
/// Locate a binary by name. First checks alongside `smellytype-osd` itself
/// (so `target/release/smellytype-osd` finds `target/release/smellytype-osd-gtk4`
