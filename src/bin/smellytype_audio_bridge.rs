//! `smellytype-audio-bridge` — sidecar that exposes the daemon's audio frame
...
//! socket (default `$XDG_RUNTIME_DIR/smellytype/audio.sock`). This bridge
...
use smellytype::audio::levels::{default_socket_path, AudioFrame, FRAME_BYTES};
...
#[derive(Parser, Debug)]
#[command(
    name = "smellytype-audio-bridge",
    version,
    about = "Bridge the smellytype daemon's audio frame socket to NDJSON on stdout"
)]
struct Args {
    /// Path to the audio-frame Unix socket. Defaults to
    /// `$XDG_RUNTIME_DIR/smellytype/audio.sock`.
    #[arg(long, env = "SMELLYTYPE_AUDIO_BRIDGE_SOCKET")]
    ...
    #[arg(
        long,
        default_value = "1",
        env = "SMELLYTYPE_AUDIO_BRIDGE_RECONNECT_SECS"
    )]
    reconnect_secs: u64,
    ...
    #[arg(
        long,
        default_value = "30",
        env = "SMELLYTYPE_AUDIO_BRIDGE_RECONNECT_MAX_SECS"
    )]
...
fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(
        ...
        "smellytype-audio-bridge starting"
    );
