//! `smellytype-osd-native` — native (SCTK + wgpu + egui-wgpu) on-screen
//! mic visualizer for the SmellyType daemon.
//!
//! Architecture:
//!
...
use smellytype::osd::config::OsdConfig;
...
use smellytype::osd::visual::PeakHold;

use crate::app::SharedState;

#[derive(Parser, Debug)]
#[command(
    name = "smellytype-osd-native",
    version,
    about = "SmellyType on-screen mic visualizer (native: SCTK + wgpu + egui-wgpu)"
)]
struct Args {
    ...
    #[arg(long, env = "SMELLYTYPE_OSD_OPACITY")]
    opacity: Option<f32>,
...
fn load_osd_config_from_file(explicit: Option<&std::path::Path>) -> OsdConfig {
    let path = explicit
        ...
        .or_else(smellytype::config::Config::default_path);
...
fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(
        "smellytype-osd-native starting; socket={:?}, size={}x{}",
        socket_path,
        osd_config.width_px,
