...
//! `SMELLYTYPE_OSD_STYLE_FILE` so QML never needs to parse user config files.
...
const PACKAGE_MANIFEST: &str = "smellytype-osd.toml";
...
) -> Result<RuntimeOsdStyle, SmellyTypeError> {
...
pub fn write_runtime_style(style: &RuntimeOsdStyle) -> Result<PathBuf, SmellyTypeError> {
    let dir = runtime_dir();
    ...
    let json = serde_json::to_string_pretty(style).map_err(|e| {
        SmellyTypeError::Config(format!("Failed to serialize Quickshell OSD style: {}", e))
    })?;
...
fn resolve_package_dir(
...
) -> Result<Option<PathBuf>, SmellyTypeError> {
    ...
    Err(SmellyTypeError::Config(format!(
...
/// values like `plugin_path = "~/.config/smellytype/osd/my-style"` work.
...
fn candidate_package_dirs(name: &str) -> Vec<PathBuf> {
    ...
    if let Ok(xdg) = std::env::var("XDG_CONFIG_HOME") {
        if !xdg.is_empty() {
            dirs.push(PathBuf::from(xdg).join("smellytype/osd").join(name));
        }
    }
    if let Some(home) = dirs::home_dir() {
        dirs.push(home.join(".config/smellytype/osd").join(name));
    }
    ...
    if let Some(home) = dirs::home_dir() {
        dirs.push(home.join(".local/share/smellytype/osd").join(name));
    }
    dirs
}

fn load_manifest(dir: &Path) -> Result<Option<OsdPackageManifest>, SmellyTypeError> {
    let path = dir.join(PACKAGE_MANIFEST);
    if !path.is_file() {
        return Ok(None);
    }
    let content = fs::read_to_string(&path).map_err(|e| {
        SmellyTypeError::Config(format!("Failed to read {}: {}", path.display(), e))
    })?;
    toml::from_str::<OsdPackageManifest>(&content)
        .map(Some)
        .map_err(|e| SmellyTypeError::Config(format!("Invalid {}: {}", path.display(), e)))
}
