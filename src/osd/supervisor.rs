...
//!
//! When `[osd] enabled = true`, the daemon spawns `smellytype-osd` as a child
...
//! after `systemctl --user restart smellytype`.
//!
//! ### `SMELLYTYPE_OSD_SUPERVISED`
//!
//! We set `SMELLYTYPE_OSD_SUPERVISED=1` on the child's environment so the
//! dispatcher (`smellytype-osd`) knows it was spawned by the daemon rather
...
const OSD_BINARY: &str = "smellytype-osd";
...
/// Spawn a tokio task that supervises `smellytype-osd`. The returned handle's
...
async fn supervise() {
    ...
    loop {
        ...
        // to kill — see #395).
        cmd.env("SMELLYTYPE_OSD_SUPERVISED", "1");
            ...
            if rapid_fails >= RAPID_FAIL_THRESHOLD {
                tracing::error!(
                    "OSD child exited {} times within {:?}. Giving up — check that \
                     smellytype-osd-gtk4 (or smellytype-osd-native) is installed and that \
