...
//! config; can be overridden via CLI flags or `SMELLYTYPE_OSD_*` env vars on
...
/// Selects which OSD frontend the `smellytype-osd` wrapper launches.
///
/// The wrapper treats this as a *preference*: if the chosen frontend's
/// binary isn't on PATH (e.g. the user built smellytype with only one of
...
impl OsdFrontend {
    ...
    pub fn binary_name(self) -> &'static str {
        match self {
            OsdFrontend::Gtk4 => "smellytype-osd-gtk4",
            OsdFrontend::Native => "smellytype-osd-native",
            OsdFrontend::Quickshell => "smellytype-osd-quickshell",
        }
    }
...
pub struct OsdConfig {
    ...
    /// the swayosd default so the smellytype OSD lands in the same band as
    ...
    /// Which OSD frontend the `smellytype-osd` wrapper launches. Defaults to
...
mod tests {
    ...
    fn frontend_binary_names() {
        assert_eq!(OsdFrontend::Gtk4.binary_name(), "smellytype-osd-gtk4");
        assert_eq!(OsdFrontend::Native.binary_name(), "smellytype-osd-native");
        assert_eq!(
