...
//!
//! Backs `smellytype config set <KEY> <VALUE>` and `smellytype config unset <KEY>`.
...
pub enum ConfigSetError {
    ...
    #[error(
        "engine '{0}' is not compiled into this binary.\n  \
         Rebuild smellytype with the matching Cargo feature:\n    \
         cargo build --release --features {0}\n  \
         Or install a prebuilt variant that includes it (see \
         `smellytype info variants`)."
    )]
    FeatureNotCompiled(String),

    #[error(
        "unknown config key '{0}'.\n  \
         Run `smellytype config schema` to list every settable key."
    )]
    UnknownKey(String),
    ...
    #[error(
        "'{key}' belongs to the '{feature}' engine, which is not compiled into \
         ...
         `smellytype info variants`) or rebuild with --features {feature}."
    )]
...
impl ConfigSetError {
    ...
    /// `smellytype config set --help`: 2 for anything the user can fix by
...
/// enum: cloud engines `soniox` and `doubao` have no local model catalog
/// and are configured through their own tables or CLI, not the TUI picker.
...
pub fn engine_feature_compiled(name: &str) -> bool {
    ...
    match engine {
        ...
        TranscriptionEngine::Soniox | TranscriptionEngine::Doubao => true,
...
mod tests {
    ...
    /// land without showing up in `smellytype config set engine <bogus>` output.
    ...
    fn set_engine_whisper_succeeds_against_full_config() {
        ...
        // file would fail validation by design — smellytype's serde struct
    ...
    fn set_engine_preserves_comments_and_adjacent_fields() {
        ...
        let marker = "\n# SMELLYTYPE-TEST-MARKER: keep this comment\n";
        ...
        assert!(
            after.contains("# SMELLYTYPE-TEST-MARKER: keep this comment"),
    ...
    fn set_key_preserves_comments() {
        ...
        let after = fs::read_to_string(&path).unwrap();
        assert!(after.contains("# SMELLYTYPE-TEST-MARKER: keep this comment"));
