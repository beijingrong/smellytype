//! `smellytype status` — read the daemon's state file, optionally render as
//! Waybar-flavoured JSON, optionally follow with inotify. The JSON shape
//! itself lives in `smellytype::status_json` (a library module) so external
...
pub(crate) async fn run_status(
    ...
    if state_file.is_none() {
        ...
        eprintln!();
        eprintln!("This enables external integrations like Waybar to monitor smellytype state.");
        std::process::exit(1);
    }
