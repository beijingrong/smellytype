//! `smellytype meeting <action>` — start/stop/pause/resume/status/list/export/show/delete/label/summarize.

use smellytype::{config, daemon_status::check_daemon_running, meeting, setup, MeetingAction};
    ...
    match action {
        MeetingAction::Start { title, diarization } => {
            ...
            if meeting_state_file.exists() {
                ...
                if state.starts_with("recording") || state.starts_with("paused") {
                    ...
                    eprintln!("Use 'smellytype meeting stop' to end it first.");
                    std::process::exit(1);
                }
            }
            ...
            if diarization.as_deref() == Some("ml") && !cfg!(feature = "ml-diarization") {
                ...
                eprintln!(
                    "  ONNX binaries (smellytype-onnx-avx2, smellytype-onnx-avx512, smellytype-onnx-cuda-*,"
                );
                eprintln!(
                    "  smellytype-onnx-migraphx). Install one of those, or omit --diarization to"
                );
            ...
            let suffix = diarization
                ...
                .unwrap_or_default();
            println!(
                "Meeting start requested{}. Check status with 'smellytype meeting status'.",
                suffix
            );
        }
        ...
        MeetingAction::Status => {
            ...
            if !meeting_state_file.exists() {
                ...
                println!();
                println!("Use 'smellytype meeting list' to see past meetings.");
                return Ok(());
            }
        ...
        MeetingAction::Show { meeting_id } => {
            match meeting::get_meeting(&meeting_config, &meeting_id) {
                Ok(meeting) => {
                    ...
                    println!();
                    println!(
                        "Use 'smellytype meeting export {}' to export the transcript.",
                        meeting_id
                    );
                }
                Err(e) => {
