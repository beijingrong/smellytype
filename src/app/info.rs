//! `smellytype info <subcommand>` — read-only introspection of the install:
...
//! `dynamic_enum` key types in `smellytype config schema --json`.

use smellytype::audio::devices;
use smellytype::config::Config;
use smellytype::config_set::{engine_feature_compiled, ENGINE_NAMES};
use smellytype::model_catalog::{self, ModelHealth, ModelVerification};
use smellytype::{setup, InfoAction};

/// Dispatch `smellytype info <subcommand>`. `config` is the already-resolved
/// configuration, so `info engines` marks the engine that would actually run
/// right now — honoring `--config` and the `SMELLYTYPE_ENGINE` override rather
...
fn run_devices(json: bool) -> anyhow::Result<()> {
    ...
    println!();
    println!("Select one with: smellytype config set audio.device <NAME>");
    Ok(())
}
...
fn run_models(json: bool, only: Option<&str>, verify: bool) -> anyhow::Result<()> {
    ...
    for (engine, statuses) in &checked {
        ...
        if statuses.iter().all(|s| s.download_arg.is_none()) {
            println!("{}  (download via: smellytype setup model)", engine);
        } else {
            println!("{}", engine);
        }
    ...
    println!();
    if any_corrupt {
        println!("Re-download a damaged model with: smellytype setup --download --model <NAME>");
    } else {
        println!("Download one with: smellytype setup --download --model <NAME>");
    }
    Ok(())
}
...
fn run_engines(json: bool, config: &Config) -> anyhow::Result<()> {
    ...
    println!();
    println!("Switch with: smellytype config set engine <NAME>");
    Ok(())
}
...
fn print_variants_text(inv: &setup::binary::Inventory) {
    ...
    // daemon is executing right now, and what /usr/bin/smellytype would launch
    ...
    if let (Some(running), Some(next)) = (inv.running_variant, inv.active_variant) {
        if running != next {
            ...
            println!("  Restart smellytype to pick up {}:", next.display());
