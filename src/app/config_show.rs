//! `smellytype config` (no subcommand) — print the resolved config to stdout,
//! including derived bits like the output-chain probe and the meeting tree.

use smellytype::{config, setup};
use std::path::PathBuf;

/// Print the models of `engine` that are installed and usable.
///
/// Goes through the same installed-model detection as `smellytype info models`
...
/// they can be copied straight into `smellytype config set <engine>.model`.
fn print_available_models(engine: &str) {
    let installed = smellytype::model_catalog::installed_models_for(engine);
...
/// Format the `[meeting]` config sections for display in `smellytype config`.
///
...
/// The smoke pipeline previously missed that `smellytype config` omitted
...
mod tests {
    ...
    /// Regression: `smellytype config` once omitted the entire `[meeting]`
