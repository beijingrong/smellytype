//! `smellytype config set` / `smellytype config unset` — small dispatchers over
//! `config_set`.
//!
//! Every write goes through `smellytype::config_set`, which shares the TUI's
...
use smellytype::{config, config_set};
...
/// the contract documented in `smellytype config set --help`.
...
fn restart_hint(restart_required: bool) {
    if restart_required {
        println!("Restart smellytype to apply: systemctl --user restart smellytype");
    }
}

/// Dispatcher for `smellytype config set <KEY> <VALUE>`.
...
/// Dispatcher for `smellytype config unset <KEY>`.
