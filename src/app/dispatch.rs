...
#[cfg(target_os = "macos")]
use smellytype::menubar;
use smellytype::{config, daemon, setup, transcribe, Cli, Commands, ConfigAction, SetupAction};
...
fn warn_if_root(command_name: &str) -> bool {
    ...
    if is_root {
        eprintln!(
            "Warning: Running 'smellytype setup {}' as root is not recommended.",
            command_name
        );
        eprintln!("  - Models will download to /root/.local/share/smellytype/ instead of your user directory");
        eprintln!(
            "  - Config changes will apply to /root/.config/smellytype/ instead of your user config"
        );
        eprintln!("  - Cannot restart your user's smellytype daemon from root");
        eprintln!();
        eprintln!("Run without sudo: smellytype setup {}", command_name);
        eprintln!();
    }
    is_root
}
...
pub(crate) async fn dispatch(
    ...
    match cli.command.unwrap_or(default_command) {
        ...
        #[cfg(target_os = "macos")]
        Commands::AppLaunch => {
            // Launched by SmellyType.app: start daemon in background, run menubar in foreground.
            ...
            let logs_dir = dirs::home_dir()
                .map(|h| h.join("Library/Logs/smellytype"))
                .unwrap_or_else(|| std::path::PathBuf::from("/tmp/smellytype"));
            ...
            let _ = std::process::Command::new("pkill")
                .args(["-9", "-f", "smellytype-bin daemon"])
                .status();
            let _ = std::process::Command::new("pkill")
                .args(["-9", "-f", "smellytype-bin menubar"])
                .status();
            let _ = std::fs::remove_file("/tmp/smellytype/smellytype.lock");
            let _ = std::fs::remove_file("/tmp/smellytype/menubar.lock");
                ...
                match engine_name.parse::<config::TranscriptionEngine>() {
                    ...
                    Err(_) => {
                        eprintln!(
                            ...
                            engine_name,
                            smellytype::cli::ENGINE_NAMES_CSV
                        );
                        std::process::exit(1);
                    }
                }
        ...
        Commands::Setup {
            ...
            match action {
                ...
                Some(SetupAction::Variant { to }) => {
                    ...
                    println!("Switched /usr/bin/smellytype to {}.", variant.binary_name());
                }
        ...
        Commands::Configure {
            ...
            if probe_audio_devices {
                smellytype::tui::probe_audio_devices();
            } else {
                ...
                smellytype::tui::run(force_package_mode, cli.config.clone())?;
            }
        }
