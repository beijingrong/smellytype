//! `smellytype record start|stop|toggle|cancel` — write override files for the
...
use smellytype::daemon::result_sidecar_path;
use smellytype::{config, daemon_status, RecordAction};
...
pub(crate) fn send_record_command(
    ...
    use smellytype::OutputModeOverride;
    ...
    // checks in `smellytype meeting` and `smellytype status`.
        ...
        RecordAction::Toggle { .. } => {
            ...
            let state_file = match config.resolve_state_file() {
                ...
                None => {
                    ...
                    eprintln!("  smellytype record start");
                    eprintln!("  smellytype record stop");
                    std::process::exit(1);
                }
            };
