...
pub(crate) async fn first_launch_setup(_config: &config::Config) {
    ...
    if !has_model {
        ...
        let _ = std::process::Command::new("osascript")
            .args([
                "-e",
                "display notification \"Downloading speech model (this may take a minute)...\" with title \"SmellyType\"",
            ])
            .status();
        ...
        match download_result {
            Ok(_) => {
                let _ = std::process::Command::new("osascript")
                    .args([
                        "-e",
                        "display notification \"Ready! Press fn to start recording.\" with title \"SmellyType\"",
                    ])
                    .status();
            }
            Err(e) => {
                ...
                let msg = format!(
                    "display notification \"Model download failed: {}. Run 'smellytype setup model' to try again.\" with title \"SmellyType\"",
                    e.to_string().replace('"', "'")
                );
                let _ = std::process::Command::new("osascript")
                    .args(["-e", &msg])
