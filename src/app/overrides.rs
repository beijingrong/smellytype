...
use smellytype::{config, setup, Cli};
...
pub(crate) fn apply_cli_overrides(config: &mut config::Config, cli: &Cli) -> Option<String> {
    ...
    if let Some(ref model) = cli.model {
        if setup::model::is_valid_model(model) {
            ...
            smellytype::notification::send_sync(
                "SmellyType: Invalid Model",
    ...
    if let Some(ref key) = cli.doubao_api_key {
        config
            .doubao
            .get_or_insert_with(config::DoubaoConfig::default)
            .api_key = Some(key.clone());
    }
    if let Some(ref id) = cli.doubao_resource_id {
        config
            .doubao
            .get_or_insert_with(config::DoubaoConfig::default)
            .resource_id = id.clone();
    }
    if let Some(seconds) = cli.doubao_final_timeout_secs {
        config
            .doubao
            .get_or_insert_with(config::DoubaoConfig::default)
            .final_timeout_secs = seconds;
    }
    // Soniox overrides
