...
use smellytype::config::schema::{self, KeyType};
use smellytype::config::Config;
use smellytype::tui::ConfigEditor;
...
pub(crate) fn run_config_schema(
    ...
    println!("SmellyType config schema v{}", schema::SCHEMA_VERSION);
    println!("  smellytype:     {}", env!("CARGO_PKG_VERSION"));
    ...
    println!("Set a value with: smellytype config set <KEY> <VALUE>");
    Ok(())
}
...
fn describe_type(ty: KeyType) -> String {
    match ty {
        ...
        KeyType::DynamicEnum { source } => {
            format!("string (see `smellytype info {} --json`)", source)
        }
