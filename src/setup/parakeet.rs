...
//! `smellytype setup onnx`/`smellytype setup parakeet` CLI.
...
/// fails. Used by detect_best_parakeet_backend to pick between smellytype-onnx-cuda-12
/// and smellytype-onnx-cuda-13 based on what the host can actually run.
...
pub fn show_status() {
    println!("=== SmellyType ONNX Engine Status ===\n");
    ...
    if available.is_empty() {
        ...
        println!("\n  Install an ONNX-enabled smellytype package to use this feature.");
    } else {
        for backend in [
    ...
    if !is_parakeet_active() && !available.is_empty() {
        ...
        println!("  sudo smellytype setup onnx --enable");
    } else if is_parakeet_active() {
        println!("To switch back to Whisper:");
        println!("  sudo smellytype setup onnx --disable");
    }
}
...
pub fn enable() -> anyhow::Result<()> {
    ...
    if available.is_empty() {
        anyhow::bail!(
            "No ONNX binaries installed.\n\
             Install an ONNX-enabled smellytype package first."
        );
    }
    ...
    println!();
    println!("Restart smellytype to use ONNX engines:");
...
pub fn disable() -> anyhow::Result<()> {
    ...
    println!(
        ...
        target.binary_name().trim_start_matches("smellytype-")
    );
    println!();
    println!("Restart smellytype to use Whisper:");
    println!("  systemctl --user restart smellytype");
...
mod tests {
    ...
    fn detect_available_backends_does_not_panic() {
        // Smoke test only — the function walks /usr/lib/smellytype/ on
