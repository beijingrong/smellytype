//! Machine-readable download progress for `smellytype setup --download`.
//!
...
/// How `smellytype setup` reports download progress.
...
/// streams like `smellytype status --follow | head` die quietly when the reader
...
/// disconnects (or a `| head -8`) kills smellytype mid-transfer. Ignoring the
...
/// Emit the terminal event for a finished `smellytype setup` run.
