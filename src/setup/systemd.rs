//! Systemd user service management for smellytype

use super::{get_smellytype_service_path, print_failure, print_info, print_success};
use std::path::PathBuf;
use tokio::process::Command;

const SERVICE_NAME: &str = "smellytype.service";
...
/// Get the path to the smellytype service file
...
fn generate_service_file() -> String {
    let smellytype_path = get_smellytype_service_path();

    format!(
        r#"[Unit]
Description=SmellyType push-to-talk voice-to-text daemon
...
[Service]
Type=simple
ExecStart={smellytype_path} daemon
Restart=on-failure
RestartSec=5
...
pub async fn install() -> anyhow::Result<()> {
    println!("Installing smellytype systemd service...\n");
    ...
    if start.success() {
        ...
        println!("    Check logs with: journalctl --user -u smellytype");
    }
    ...
    println!("  systemctl --user status smellytype   # Check status");
    println!("  systemctl --user restart smellytype  # Restart");
    println!("  systemctl --user stop smellytype     # Stop");
    println!("  journalctl --user -u smellytype -f   # View logs");
...
pub async fn uninstall() -> anyhow::Result<()> {
    println!("Uninstalling smellytype systemd service...\n");
