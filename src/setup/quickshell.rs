//! Quickshell QML tree installer for smellytype.
//!
//! Copies the Quickshell QML tree (`shell.qml`, `OsdSurface.qml`,
//! `EnginePicker.qml`, `MeetingControls.qml`, and the `smellytype-shared/`
//! module) into the user's data directory so the `smellytype-osd-quickshell`
//! launcher can find it. Also symlinks the `smellytype-audio-bridge` sidecar
...
//! toggle via flag files in `$XDG_RUNTIME_DIR/smellytype/`.
//!
//! Source tree resolution (first match wins):
//! 1. `--source <DIR>` CLI override
//! 2. `$SMELLYTYPE_QUICKSHELL_SOURCE_DIR` env var
//! 3. `<binary's dir>/../share/smellytype/quickshell/` (installed layout)
//! 4. `/usr/share/smellytype/quickshell/`
//! 5. `quickshell/` relative to current working directory (dev-from-repo-root)
//!
//! Destination: `$XDG_DATA_HOME/smellytype/quickshell/` (or
//! `~/.local/share/smellytype/quickshell/` if `XDG_DATA_HOME` is unset).
//!
//! Bridge binary resolution (first match wins):
//! 1. `--bridge <PATH>` CLI override
//! 2. `$SMELLYTYPE_AUDIO_BRIDGE_BINARY` env var
//! 3. `<binary's dir>/../lib/smellytype/smellytype-audio-bridge` (installed layout)
//! 4. `/usr/lib/smellytype/smellytype-audio-bridge`
//! 5. `which smellytype-audio-bridge` (already on PATH — no symlink needed)
//! 6. `target/release/smellytype-audio-bridge` / `target/debug/...` (dev convenience)
//!
//! Bridge symlink target defaults to `$XDG_BIN_HOME/smellytype-audio-bridge`
//! (or `~/.local/bin/smellytype-audio-bridge`). Refuses to write outside
//! `$HOME` without `--force`.
...
const QUICKSHELL_SUBDIR: &str = "smellytype/quickshell";

/// Files at the root of the QML tree that are part of the install.
const ROOT_FILES: &[&str] = &[
    "shell.qml",
    "OsdSurface.qml",
    "TranscriptPreview.qml",
    "EnginePicker.qml",
    "MeetingControls.qml",
    "README.md",
];

/// Files in the `smellytype-shared/` subdirectory.
...
const SHARED_SUBDIR: &str = "smellytype-shared";

/// Resolve the default install target.
///
/// Returns `$XDG_DATA_HOME/smellytype/quickshell` if `XDG_DATA_HOME` is set
/// and non-empty, otherwise `~/.local/share/smellytype/quickshell`.
...
pub fn resolve_source_dir(cli_source: Option<&Path>) -> Option<PathBuf> {
    ...
    if let Ok(env_path) = env::var("SMELLYTYPE_QUICKSHELL_SOURCE_DIR") {
    ...
    if let Ok(exe) = env::current_exe() {
        if let Some(parent) = exe.parent() {
            let installed = parent.join("../share/smellytype/quickshell");
    ...
    let system = PathBuf::from("/usr/share/smellytype/quickshell");
...
/// QML files, the `smellytype-shared/` module, and the README files. Returns
/// the list of relative paths that were written.
pub fn install_tree(
    source: &Path,
    target: &Path,
    force: bool,
) -> Result<Vec<PathBuf>, SmellyTypeError> {
    if target.exists() && !force && dir_has_entries(target) {
        return Err(SmellyTypeError::Config(format!(
    ...
    fs::create_dir_all(target).map_err(|e| {
        SmellyTypeError::Config(format!("Failed to create {}: {}", target.display(), e))
    })?;
    ...
    for (src, rel) in &files {
        ...
        if let Some(parent) = dst.parent() {
            fs::create_dir_all(parent).map_err(|e| {
                SmellyTypeError::Config(format!("Failed to create {}: {}", parent.display(), e))
            })?;
        }
        fs::copy(src, &dst).map_err(|e| {
            SmellyTypeError::Config(format!(
                "Failed to copy {} -> {}: {}",
                src.display(),
                dst.display(),
                e
...
const BRIDGE_BIN_NAME: &str = "smellytype-audio-bridge";
...
pub enum BridgeSource {
    ...
    Path(PathBuf),
    /// `which smellytype-audio-bridge` succeeded at this path; PATH already has it.
    OnPath(PathBuf),
}

/// Resolve the smellytype-audio-bridge source binary.
///
...
pub fn resolve_bridge_source(cli_bridge: Option<&Path>) -> Option<BridgeSource> {
    ...
    if let Ok(env_path) = env::var("SMELLYTYPE_AUDIO_BRIDGE_BINARY") {
        if !env_path.is_empty() {
            let p = PathBuf::from(env_path);
            if p.is_file() {
    ...
    if let Ok(exe) = env::current_exe() {
        if let Some(parent) = exe.parent() {
            let installed = parent.join("../lib/smellytype").join(BRIDGE_BIN_NAME);
    ...
    let system = PathBuf::from("/usr/lib/smellytype").join(BRIDGE_BIN_NAME);
    if system.is_file() {
...
pub fn install_bridge_symlink(
    ...
    target: &Path,
    force: bool,
) -> Result<BridgeInstallOutcome, SmellyTypeError> {
...
pub fn print_bindings() {
    ...
    println!("bind = SUPER, E, exec, mkdir -p $XDG_RUNTIME_DIR/smellytype && touch $XDG_RUNTIME_DIR/smellytype/engine-picker.flag");
    println!("bind = SUPER, M, exec, mkdir -p $XDG_RUNTIME_DIR/smellytype && touch $XDG_RUNTIME_DIR/smellytype/meeting-controls.flag");
    println!();

    println!("# Sway (~/.config/sway/config)");
    println!("bindsym $mod+e exec mkdir -p $XDG_RUNTIME_DIR/smellytype && touch $XDG_RUNTIME_DIR/smellytype/engine-picker.flag");
    println!("bindsym $mod+m exec mkdir -p $XDG_RUNTIME_DIR/smellytype && touch $XDG_RUNTIME_DIR/smellytype/meeting-controls.flag");
    println!();

    println!("# River (~/.config/river/init)");
    println!("riverctl map normal Super E spawn 'mkdir -p $XDG_RUNTIME_DIR/smellytype && touch $XDG_RUNTIME_DIR/smellytype/engine-picker.flag'");
    println!("riverctl map normal Super M spawn 'mkdir -p $XDG_RUNTIME_DIR/smellytype && touch $XDG_RUNTIME_DIR/smellytype/meeting-controls.flag'");
    println!();

    println!(
        "The Quickshell-based OSD activates automatically when smellytype is recording \
         - no compositor binding needed."
    );
}

/// Run the full `smellytype setup quickshell` flow.
...
pub fn run(
    ...
    let source_dir = resolve_source_dir(source.as_deref()).ok_or_else(|| {
        SmellyTypeError::Config(
            "Could not find the Quickshell QML source tree.\n\
             Searched (in order):\n  \
             --source <DIR>\n  \
             $SMELLYTYPE_QUICKSHELL_SOURCE_DIR\n  \
             <binary>/../share/smellytype/quickshell/\n  \
             /usr/share/smellytype/quickshell/\n  \
             ./quickshell/\n\
    ...
    let bridge_summary = if skip_bridge {
        println!();
        println!("Skipping smellytype-audio-bridge install (--skip-bridge).");
        None
    } else {
        println!();
        match install_bridge(bridge.as_deref(), bridge_target.as_deref(), force)? {
            Some((outcome, link_target, link_source)) => match outcome {
                ...
                BridgeInstallOutcome::AlreadyLinked => {
                    println!(
                        "smellytype-audio-bridge already linked at {} (no change).",
                        link_target.display()
                    );
                    Some(link_target)
                }
                BridgeInstallOutcome::AlreadyOnPath(path) => {
                    println!(
                        "smellytype-audio-bridge is already on PATH at {} (no symlink needed).",
                        path.display()
                    );
                    None
                }
            ...
            None => {
                println!(
                    "Could not locate smellytype-audio-bridge. The QML waveform will\n  \
                     stay empty until the bridge is on PATH. Searched (in order):\n  \
                     --bridge <PATH>\n  \
                     $SMELLYTYPE_AUDIO_BRIDGE_BINARY\n  \
                     <binary>/../lib/smellytype/smellytype-audio-bridge\n  \
                     /usr/lib/smellytype/smellytype-audio-bridge\n  \
                     `which smellytype-audio-bridge`\n  \
                     ./target/release/smellytype-audio-bridge\n  \
                     ./target/debug/smellytype-audio-bridge"
                );
                None
            }
        }
    ...
    if let Some(link) = bridge_summary.as_ref() {
        println!(
            "smellytype-audio-bridge linked at {}. Make sure ~/.local/bin is on your\n  \
             PATH (most shells already include it).",
            link.display()
        );
    }
    println!(
        "Run with: smellytype-osd-quickshell  (or set [osd] frontend = \"quickshell\" in config.toml)"
    );
...
fn install_bridge(
    ...
    force: bool,
) -> Result<Option<(BridgeInstallOutcome, PathBuf, PathBuf)>, SmellyTypeError> {
    ...
    println!(
        "smellytype-audio-bridge target: {}",
        resolved_target.display()
    );
...
mod tests {
    ...
    fn resolve_source_dir_prefers_cli_override() {
        ...
        unsafe {
            env::set_var("SMELLYTYPE_QUICKSHELL_SOURCE_DIR", bogus.path());
        }
        ...
        unsafe {
            env::remove_var("SMELLYTYPE_QUICKSHELL_SOURCE_DIR");
        }
    }
    ...
    fn resolve_source_dir_honors_env_var() {
        ...
        unsafe {
            env::set_var("SMELLYTYPE_QUICKSHELL_SOURCE_DIR", valid.path());
        }
    ...
    fn resolve_bridge_source_prefers_cli_override() {
        ...
        let bin = dir.path().join("smellytype-audio-bridge");
    ...
    fn resolve_bridge_source_honors_env_var() {
        ...
        unsafe {
            env::set_var("SMELLYTYPE_AUDIO_BRIDGE_BINARY", &bin);
        }
        let resolved = resolve_bridge_source(None).unwrap();
        unsafe {
            env::remove_var("SMELLYTYPE_AUDIO_BRIDGE_BINARY");
        }
        assert_eq!(resolved, BridgeSource::Path(bin));
    }
    ...
    fn resolve_bridge_source_returns_none_when_missing() {
        ...
        // /usr/lib/smellytype/... only if it actually exists on disk.
        unsafe {
            env::remove_var("SMELLYTYPE_AUDIO_BRIDGE_BINARY");
        }
        ...
        let bogus = PathBuf::from("/nonexistent/path/smellytype-audio-bridge");
    ...
    fn install_bridge_symlink_creates_link() {
        ...
        let bin = src_dir.path().join("smellytype-audio-bridge");
        make_fake_bridge(&bin);
        ...
        let target = fake_home.path().join(".local/bin/smellytype-audio-bridge");
    ...
    fn install_bridge_symlink_refuses_overwrite_without_force() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let src_dir = TempDir::new().unwrap();
        let bin_a = src_dir.path().join("smellytype-audio-bridge");
    ...
    fn install_bridge_symlink_refuses_outside_home_without_force() {
        ...
        let target = outside.path().join("smellytype-audio-bridge");
    ...
    fn default_bridge_target_honors_xdg_bin_home() {
        ...
        unsafe {
            env::set_var("XDG_BIN_HOME", "/tmp/smellytype-test-bin");
        }
        let dir = default_bridge_target();
        assert_eq!(
            dir,
            PathBuf::from("/tmp/smellytype-test-bin/smellytype-audio-bridge")
        );
        unsafe {
            match prev {
    ...
    fn run_with_skip_bridge_does_not_touch_target() {
        ...
        assert!(
            !bin_home.join("smellytype-audio-bridge").exists(),
    ...
    fn default_target_dir_honors_xdg_data_home() {
        ...
        unsafe {
            env::set_var("XDG_DATA_HOME", "/tmp/smellytype-test-xdg");
        }
        let dir = default_target_dir();
        assert_eq!(
            dir,
            PathBuf::from("/tmp/smellytype-test-xdg/smellytype/quickshell")
        );
        unsafe {
            match prev {
