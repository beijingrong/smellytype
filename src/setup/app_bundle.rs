...
//!
//! Creates a proper macOS app bundle for smellytype and manages Login Items.
...
use super::{get_smellytype_path, print_failure, print_info, print_success, print_warning};

const APP_NAME: &str = "SmellyType.app";
pub const BUNDLE_ID: &str = "io.smellytype.daemon";
...
fn logs_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join("Library/Logs/smellytype"))
}
...
<dict>
    ...
    <string>smellytype-bin</string>
    <key>CFBundleIdentifier</key>
    <string>{bundle_id}</string>
    <key>CFBundleName</key>
    <string>SmellyType</string>
    ...
    <string>SmellyType needs microphone access for speech-to-text transcription.</string>
    <key>NSAppleEventsUsageDescription</key>
    <string>SmellyType needs accessibility access to type transcribed text.</string>
    <key>NSInputMonitoringUsageDescription</key>
    <string>SmellyType monitors keyboard input to detect your push-to-talk hotkey.</string>
</dict>
</plist>
"#,
        bundle_id = BUNDLE_ID,
...
pub fn create_app_bundle() -> anyhow::Result<()> {
    ...
    // Copy the current smellytype binary (handle self-copy case)
    let source_binary = get_smellytype_path();
    let dest_binary = macos_path.join("smellytype-bin");
    let source_canon =
    ...
    if binary_replaced {
        ...
        let temp_binary = macos_path.join("smellytype-bin.tmp");
    ...
    let wrapper_path = macos_path.join("smellytype");
...
pub fn add_to_login_items() -> anyhow::Result<bool> {
    ...
    if not (exists login item "SmellyType") then
...
pub fn remove_from_login_items() -> anyhow::Result<bool> {
    ...
    if exists login item "SmellyType" then
        delete login item "SmellyType"
    end if
end tell"#;
...
pub fn is_in_login_items() -> bool {
    let script = r#"tell application "System Events"
    return exists login item "SmellyType"
end tell"#;
...
pub async fn install() -> anyhow::Result<()> {
    println!("Installing SmellyType.app...\n");
    ...
    if launched {
        print_success("Launched SmellyType.app");
    } else {
        print_warning("Could not launch automatically");
        print_info("Start manually: open /Applications/SmellyType.app");
    }

    println!("\n---");
    println!("\x1b[32m✓ Installation complete!\x1b[0m");
    println!();
    println!("\x1b[1mIMPORTANT: Grant permissions to SmellyType.app:\x1b[0m");
    println!();
    println!("  1. System Settings > Privacy & Security > \x1b[1mAccessibility\x1b[0m");
    println!("     Add and enable SmellyType");
    println!();
    ...
    println!("     SmellyType should appear after first use - enable it");
    println!();
    println!("SmellyType will start automatically on login.");
...
pub async fn uninstall() -> anyhow::Result<()> {
    println!("Uninstalling SmellyType.app...\n");
    ...
    if app_bundle_path().exists() {
        remove_app_bundle()?;
        print_success("Removed SmellyType.app");
    } else {
        print_info("SmellyType.app was not installed");
    }
...
pub async fn status() -> anyhow::Result<()> {
    ...
    if app_bundle_path().exists() {
        ...
        print_failure("SmellyType.app not installed");
        print_info("Install with: smellytype setup app-bundle");
        return Ok(());
    }
    ...
    if is_in_login_items() {
        ...
        print_info("Add with: smellytype setup app-bundle");
    }

    // Check if running
    let output = Command::new("pgrep")
        .args(["-f", "SmellyType.app"])
        .output();
    ...
    match output {
        ...
        _ => {
            ...
            print_info("Start with: open /Applications/SmellyType.app");
        }
    }
