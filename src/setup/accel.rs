...
//!
//! `smellytype info accel` answers that from observed facts, never from config.
...
//! loaded a model onto a GPU. Every GPU stack smellytype uses falls back to CPU
...
//!    state to `$XDG_RUNTIME_DIR/smellytype/accel.json` when it loads a model
...
//!    `/proc/<pid>/exe` rather than the `/usr/bin/smellytype` symlink, since the
...
impl AccelState {
    ...
    pub const fn explanation(self) -> &'static str {
        match self {
            ...
            AccelState::CpuOnly => {
                "This binary has no GPU support. Switch variants with: smellytype setup variant --to <NAME>"
            }
            AccelState::Unknown => {
                "Not determinable. Transcribe once so the daemon loads a model, then re-run."
            }
            AccelState::NotRunning => {
                "No daemon is running. Start it with: systemctl --user start smellytype"
            }
        }
    }
}

/// What `smellytype info accel` reports.
#[derive(Debug, Clone)]
...
/// Contents of `$XDG_RUNTIME_DIR/smellytype/accel.json`.
///
...
pub(crate) fn classify_line(line: &str) -> Option<(Signal, Option<&'static str>)> {
    ...
    // smellytype's own "about to try this EP" lines. Intent only: they are
...
/// 3094859 smellytype-onnx-mi 1       392437760       0               UNKNOWN
/// ```
...
///
/// Resolved through `/proc/<pid>/exe`, not the `/usr/bin/smellytype` symlink: the
...
fn running_variant(pid: i32) -> (Option<Variant>, Option<String>) {
    ...
    match exe {
        Some(path) => {
            ...
            let note = match variant {
                Some(v) => format!(
                    "binary: {} (variant {})",
                    path.display(),
                    v.binary_name().trim_start_matches("smellytype-")
                ),
                None => format!(
        ...
        None => (
            ...
            Some(
                "binary: could not read /proc/<pid>/exe; fell back to the /usr/bin/smellytype symlink"
                    .to_string(),
            ),
        ),
    }
...
";
    ...
    /// End to end against a live process that is not a smellytype daemon: the
