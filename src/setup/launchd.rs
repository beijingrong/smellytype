...
//! smellytype as a launchd user service on macOS.

use super::{get_smellytype_path, print_failure, print_info, print_success, print_warning};
use std::fs;
use std::path::PathBuf;

const PLIST_FILENAME: &str = "io.smellytype.daemon.plist";
...
<dict>
    <key>Label</key>
    <string>io.smellytype.daemon</string>

    <key>ProgramArguments</key>
    <array>
        <string>{smellytype_path}</string>
...
"#,
        smellytype_path = smellytype_path,
...
pub async fn install() -> anyhow::Result<()> {
    println!("Installing SmellyType LaunchAgent...\n");

    // Check if we're on macOS
    if !cfg!(target_os = "macos") {
        print_failure("This command is only available on macOS");
        print_info("On Linux, use: smellytype setup systemd");
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        println!();
        print_info("Recommended: use 'smellytype setup app-bundle' instead.");
    ...
    match status {
        ...
        _ => {
            ...
            print_info("Try: launchctl load ~/Library/LaunchAgents/io.smellytype.daemon.plist");
        }
    }
    ...
    println!();
    println!("SmellyType will now start automatically on login.");
    println!();
    println!("Useful commands:");
    println!("  launchctl start io.smellytype.daemon   - Start now");
    println!("  launchctl stop io.smellytype.daemon    - Stop");
    println!("  launchctl unload ~/Library/LaunchAgents/io.smellytype.daemon.plist - Disable");
    println!();
    println!("Logs:");
    if let Some(logs) = logs_dir() {
...
pub async fn uninstall() -> anyhow::Result<()> {
    println!("Uninstalling SmellyType LaunchAgent...\n");
...
pub async fn status() -> anyhow::Result<()> {
    println!("SmellyType LaunchAgent Status\n");
    println!("==========================\n");
    ...
    if plist.exists() {
        ...
        print_info("Install with: smellytype setup launchd");
        return Ok(());
    }

    // Check if service is running
    let output = std::process::Command::new("launchctl")
        .args(["list", "io.smellytype.daemon"])
        .output();

    match output {
        Ok(out) if out.status.success() => {
            let stdout = String::from_utf8_lossy(&out.stdout);
            if stdout.contains("io.smellytype.daemon") {
        ...
        _ => {
            ...
            print_info(
                "Start with: launchctl load ~/Library/LaunchAgents/io.smellytype.daemon.plist",
            );
        }
    }
