//! Waybar configuration generation for smellytype
...
pub fn install() -> Result<(), SmellyTypeError> {
    ...
    if !config_path.exists() {
        ...
        if !alt_path.exists() {
            ...
            return Err(SmellyTypeError::Config("Waybar config not found".into()));
        }
    }
    ...
    let config_content = fs::read_to_string(&config_path)
        .map_err(|e| SmellyTypeError::Config(format!("Failed to read waybar config: {}", e)))?;

    // Check if smellytype module already exists
    if config_content.contains("\"custom/smellytype\"") {
        println!("SmellyType module already exists in Waybar config.");
    ...
    // Create backup
    let backup_path = format!("{}.smellytype-backup", config_path.display());
    fs::copy(&config_path, &backup_path)
        .map_err(|e| SmellyTypeError::Config(format!("Failed to create backup: {}", e)))?;
    ...
    println!("Added smellytype module to Waybar config.");

    // Add CSS if style file exists and doesn't already have smellytype styles
    if style_path.exists() {
        let style_content = fs::read_to_string(&style_path).unwrap_or_default();
        if !style_content.contains("#custom-smellytype") {
            let mut file = fs::OpenOptions::new()
                .append(true)
                .open(&style_path)
                .map_err(|e| SmellyTypeError::Config(format!("Failed to open style.css: {}", e)))?;
            writeln!(file, "\n{}", get_css_config())
                .map_err(|e| SmellyTypeError::Config(format!("Failed to write CSS: {}", e)))?;
            println!("Added smellytype styling to Waybar CSS.");
        }
    }

    println!("\nWaybar integration installed successfully!");
    println!("Restart Waybar to see the smellytype status widget:");
...
pub fn uninstall() -> Result<(), SmellyTypeError> {
    ...
    if !config_content.contains("\"custom/smellytype\"") {
        println!("SmellyType module not found in Waybar config.");
        return Ok(());
    }

    // Prompt for confirmation
    print!("Remove smellytype from Waybar configuration? [y/N] ");
    io::stdout().flush().unwrap();
    ...
    fs::write(&config_path, new_config)
        .map_err(|e| SmellyTypeError::Config(format!("Failed to write config: {}", e)))?;
    println!("Removed smellytype module from Waybar config.");
    ...
    if style_path.exists() {
        ...
        if style_content.contains("#custom-smellytype") {
            ...
            fs::write(&style_path, new_style).map_err(|e| {
                SmellyTypeError::Config(format!("Failed to write style.css: {}", e))
            })?;
            println!("Removed smellytype styling from Waybar CSS.");
        }
    }
...
/// Inject the smellytype module into waybar config content
fn inject_module_into_config(content: &str) -> Result<String, SmellyTypeError> {
    let mut result = content.to_string();

    // Add "custom/smellytype" to modules-right array
    ...
    if let Some(pos) = result.find("\"modules-right\"") {
        if let Some(bracket_pos) = result[pos..].find('[') {
            ...
            result.insert_str(insert_pos, "\"custom/smellytype\",\n        ");
        }
    ...
    "custom/smellytype": {
        "exec": "smellytype status --follow --format json",
        "return-type": "json",
        "format": "{}",
        "tooltip": true,
        "on-click": "systemctl --user restart smellytype"
    }
"#
        } else {
            r#"
...
/// Remove the smellytype module from waybar config content
fn remove_module_from_config(content: &str) -> String {
    ...
    // This distinguishes from array references like ["custom/smellytype", ...]
    let definition_pattern = regex::Regex::new(r#""custom/smellytype"\s*:"#).unwrap();
    if let Some(mat) = definition_pattern.find(&result) {
        let start = mat.start();
        // Find the opening { after "custom/smellytype":
    ...
    // Now remove "custom/smellytype" from module arrays (modules-right, modules-left, etc.)
    // This handles the reference that was added to the array
    result = regex::Regex::new(r#""custom/smellytype",?\s*\n?\s*"#)
        .unwrap()
        .replace_all(&result, "")
        .to_string();
...
/// Remove smellytype CSS from style content
fn remove_css_from_style(content: &str) -> String {
    let mut result = content.to_string();

    // Remove all #custom-smellytype CSS blocks using brace matching
    while let Some(start) = result.find("#custom-smellytype") {
    ...
    // Remove any commented out smellytype blocks
...
pub fn print_config() {
    println!("Waybar Configuration for SmellyType\n");
    println!("================================\n");

    println!("1. Add this to your Waybar config (usually ~/.config/waybar/config):\n");
    println!("   In the \"modules-right\" (or left/center) array, add: \"custom/smellytype\"\n");
   ...
   @keyframes pulse {{
    ...
    println!("\n\n3. Enable state file in smellytype config (~/.config/smellytype/config.toml):\n");
    ...
    println!("------------------");
    println!(
        "SmellyType outputs an \"alt\" field in JSON that enables Waybar's format-icons feature."
    );
    ...
    println!("\nAlternatively, configure icons in smellytype's config.toml:\n");
    println!("   [status]");
