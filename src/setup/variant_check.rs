...
//!
//! `/usr/bin/smellytype` dispatches to one installed variant (CPU Whisper,
...
//! and `smellytype models` lists "rebuild with --features X" for every
...
pub struct VariantMismatch {
    ...
    /// `"smellytype-avx2"`. `None` only on source builds (no wrapper).
    pub active_variant_name: Option<String>,
    /// Recommended fix; varies by install kind. See [`Remediation`].
    pub remediation: Remediation,
}

/// How to fix the mismatch, tailored to how smellytype was installed.
///
...
pub enum Remediation {
    /// Package install (smellytype-bin / .deb / .rpm / Nix). Swap the
    /// `/usr/bin/smellytype` dispatch to the listed variant. `target` is the
...
pub fn required_feature(engine: TranscriptionEngine) -> Option<&'static str> {
    match engine {
        TranscriptionEngine::Whisper | TranscriptionEngine::Doubao => None,
...
/// at what's installed under `/usr/lib/smellytype/`. A package install with the
...
/// smellytype` invokes the same wrapper. Surfacing this is the whole point of
...
mod tests {
    ...
    fn fake_inventory(
        ...
        Inventory {
            install_kind,
            binary_path: "/usr/bin/smellytype".into(),
            package_lib_dir: Some("/usr/lib/smellytype".into()),
    ...
    fn parakeet_on_cpu_whisper_binary_is_mismatch() {
        ...
        assert_eq!(m.active_variant_name.as_deref(), Some("smellytype-avx2"));
        match m.remediation {
