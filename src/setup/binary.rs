//! Engine-agnostic smellytype binary inventory and switching.
//!
//! SmellyType ships seven prebuilt variants in `/usr/lib/smellytype/` (Whisper:
//! avx2/avx512/vulkan; ONNX: avx2/avx512/cuda/migraphx). `/usr/bin/smellytype` is a
//! symlink into that directory, and switching engines means updating that
//! symlink.
//!
//! Source builds typically live at `/usr/local/bin/smellytype` or `~/.cargo/bin/`
...
pub const LIB_DIR: &str = "/usr/lib/smellytype";
pub const SYSTEM_BIN: &str = "/usr/bin/smellytype";

/// Install `/usr/bin/smellytype` so it dispatches to `binary_path`. CPU-only
/// variants get a plain symlink; GPU/ONNX variants whose binary lives in
/// a /usr/lib/smellytype/<variant>/ subdirectory next to companion ONNX
...
/// at /usr/bin/smellytype leaves argv[0] = "/usr/bin/smellytype", so ORT
...
/// /usr/lib/smellytype/smellytype-onnx-migraphx) or the canonical real path;
...
/// loader path from argv[0], so /usr/bin/smellytype has to be a script
...
/// Return the subdirectory under /usr/lib/smellytype/ where a wrapper-needing
...
fn variant_subdir(basename: &str) -> Option<&'static str> {
    match basename {
        "smellytype-onnx-cuda-12" => Some("cuda-12"),
        "smellytype-onnx-cuda-13" => Some("cuda-13"),
        "smellytype-onnx-migraphx" => Some("migraphx"),
        _ => None,
    }
}
...
/// `Try: sudo smellytype <retry_hint>`.
...
pub fn install_active_binary(
    ...
    // input path — whose parent is `/usr/lib/smellytype/` rather than the
    // variant subdir like `migraphx/`. Result: a symlink got written at
    // /usr/bin/smellytype instead of the wrapper, and ORT's argv[0]-based
    ...
    // (/usr/lib/smellytype/<subdir>/<basename>) so the wrapper still points
    ...
    if needs_wrapper {
        ...
        // to $XDG_CACHE_HOME/smellytype/migraphx, honoring any user override.
        let is_migraphx = basename == "smellytype-onnx-migraphx";
        let migraphx_env = if is_migraphx {
            "\
             : \"${ORT_MIGRAPHX_MODEL_CACHE_PATH:=${XDG_CACHE_HOME:-$HOME/.cache}/smellytype/migraphx}\"\n\
...
/// Read /usr/bin/smellytype and return the canonical real binary it dispatches
...
pub enum Acceleration {
    ...
    /// Runtime. Replaced ROCm in 0.7.0; old `smellytype-onnx-rocm` binary names
...
/// Every binary name smellytype recognizes in `/usr/lib/smellytype/`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
...
impl Variant {
    ...
    pub const fn binary_name(self) -> &'static str {
        match self {
            Variant::WhisperAvx2 => "smellytype-avx2",
            Variant::WhisperAvx512 => "smellytype-avx512",
            Variant::WhisperVulkan => "smellytype-vulkan",
            Variant::WhisperNative => "smellytype-native",
            Variant::OnnxAvx2 => "smellytype-onnx-avx2",
            Variant::OnnxAvx512 => "smellytype-onnx-avx512",
            Variant::OnnxCuda12 => "smellytype-onnx-cuda-12",
            Variant::OnnxCuda13 => "smellytype-onnx-cuda-13",
            Variant::OnnxCuda => "smellytype-onnx-cuda",
            Variant::OnnxMigraphx => "smellytype-onnx-migraphx",
            Variant::OnnxNative => "smellytype-onnx",
        }
    }
    ...
    /// Reverse lookup. Accepts current names plus legacy `smellytype-parakeet*`
    /// names from before the ONNX rename.
    pub fn from_binary_name(name: &str) -> Option<Self> {
        match name {
            "smellytype-avx2" => Some(Variant::WhisperAvx2),
            "smellytype-avx512" => Some(Variant::WhisperAvx512),
            "smellytype-vulkan" => Some(Variant::WhisperVulkan),
            "smellytype-native" => Some(Variant::WhisperNative),
            "smellytype-onnx-avx2" | "smellytype-parakeet-avx2" => Some(Variant::OnnxAvx2),
            "smellytype-onnx-avx512" | "smellytype-parakeet-avx512" => Some(Variant::OnnxAvx512),
            "smellytype-onnx-cuda-12" => Some(Variant::OnnxCuda12),
            "smellytype-onnx-cuda-13" => Some(Variant::OnnxCuda13),
            "smellytype-onnx-cuda" | "smellytype-parakeet-cuda" => Some(Variant::OnnxCuda),
            // Canonical name for the MIGraphX-based ONNX binary (0.7.0+).
            "smellytype-onnx-migraphx" => Some(Variant::OnnxMigraphx),
            // Legacy ROCm names continue to resolve during the symlink-compat
            // window so `smellytype-bin` users with the old AUR symlink still see
            // the variant correctly identified.
            "smellytype-onnx-rocm" | "smellytype-parakeet-rocm" => Some(Variant::OnnxMigraphx),
            "smellytype-onnx" | "smellytype-parakeet" => Some(Variant::OnnxNative),
            _ => None,
        }
    }
}
...
pub enum InstallKind {
    /// `/usr/bin/smellytype` resolves into `/usr/lib/smellytype/`. Switching is
    /// supported by rewriting that symlink.
    Package,
    /// The running binary lives outside `/usr/lib/smellytype/`. Single binary,
...
pub struct Inventory {
    ...
    /// What `/usr/bin/smellytype` would launch next. Not necessarily what is
    ...
    /// than `/usr/bin/smellytype` launched the daemon.
...
pub struct Recommendation {
    ...
    /// the Whisper recommendation since smellytype's default engine is Whisper.
    pub primary: Variant,
}
...
/// Read the `/usr/bin/smellytype` symlink to learn which packaged variant is
...
/// resolves `/usr/bin/smellytype`, which describes the *next* process to start:
...
pub fn active_variant() -> Option<Variant> {
    // Handle both shapes /usr/bin/smellytype can take: a symlink (CPU
    // variants) or a wrapper script (GPU/ONNX variants whose binary
    // lives in a /usr/lib/smellytype/<variant>/ subdir alongside companion
...
///
/// Drives the "Features:" line in `smellytype info variants`, the same field
...
/// validator emitted a spurious "rebuild smellytype with the corresponding
...
pub fn compiled_features() -> Vec<&'static str> {
    ...
    // OSD frontends. Affects whether `smellytype-osd-gtk4` and
    // `smellytype-osd-native` are present in the build. (The Quickshell
...
/// Rewrite `/usr/bin/smellytype` to point at the requested variant's binary.
...
pub fn switch_to(variant: Variant) -> anyhow::Result<()> {
    ...
    if !binary_path.exists() {
        anyhow::bail!(
            "Binary not found: {}\n\
             Install the appropriate smellytype package variant.",
            binary_path.display()
        );
    }
...
mod tests {
    ...
    fn legacy_parakeet_names_resolve() {
        assert_eq!(
            Variant::from_binary_name("smellytype-parakeet-avx2"),
            Some(Variant::OnnxAvx2)
        );
        assert_eq!(
            Variant::from_binary_name("smellytype-parakeet-cuda"),
            Some(Variant::OnnxCuda)
        );
        assert_eq!(
            Variant::from_binary_name("smellytype-parakeet"),
    ...
    fn unknown_binary_name_is_none() {
        assert_eq!(Variant::from_binary_name("smellytype-totally-fake"), None);
    ...
    fn detect_install_kind_classifies_package_vs_source() {
        assert_eq!(
            detect_install_kind(Path::new("/usr/lib/smellytype/smellytype-avx2")),
            InstallKind::Package
        );
        assert_eq!(
            detect_install_kind(Path::new("/usr/local/bin/smellytype")),
            InstallKind::Source
        );
        assert_eq!(
            detect_install_kind(Path::new("/home/user/.cargo/bin/smellytype")),
            InstallKind::Source
        );
    }
    ...
    /// users in `smellytype info variants` and the TUI inventory panes, and
    ...
    fn required_glibc_picks_the_highest_version() {
        let stderr = "smellytype: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.38' not found (required by smellytype)\n\
                      smellytype: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39' not found (required by smellytype)\n";
    ...
    fn verify_binary_runs_rejects_a_non_executable() {
        ...
        let path = dir.path().join("smellytype-broken");
    ...
    fn verify_binary_runs_accepts_a_working_binary() {
        ...
        let dir = tempfile::TempDir::new().unwrap();
        let path = dir.path().join("smellytype-fake");
    ...
    /// with the top-level alias path (`/usr/lib/smellytype/smellytype-onnx-migraphx`,
    ...
    /// `/usr/lib/smellytype/` parent and emitted a symlink instead of the
    ...
    fn wrapper_decision_uses_basename_not_canonical_parent() {
        ...
        for variant in [
            ...
            let path = std::path::Path::new("/nonexistent/usr/lib/smellytype").join(variant);
    ...
    /// should reconstruct `/usr/lib/smellytype/<subdir>/<basename>` for
    ...
    fn variant_subdir_reconstructs_expected_path() {
        let cases = [
            ("smellytype-onnx-cuda-12", "cuda-12"),
            ("smellytype-onnx-cuda-13", "cuda-13"),
            ("smellytype-onnx-migraphx", "migraphx"),
        ];
        ...
        assert_eq!(variant_subdir("smellytype-avx2"), None);
        assert_eq!(variant_subdir("smellytype-vulkan"), None);
    ...
    fn running_variant_reads_the_live_process_not_the_symlink() {
        ...
        // whatever /usr/bin/smellytype happens to point at.
