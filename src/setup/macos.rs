...
fn check_app_bundle() -> bool {
    ...
    let binary_path = app_path.join("Contents/MacOS/smellytype");
    let info_plist = app_path.join("Contents/Info.plist");

    app_path.exists() && binary_path.exists() && info_plist.exists()
}

/// Reset TCC permissions for SmellyType (forces re-prompt)
...
fn check_notification_icon() -> bool {
    let candidates = [
        dirs::data_dir().map(|d| d.join("smellytype/icon.png")),
        dirs::config_dir().map(|d| d.join("smellytype/icon.png")),
    ];
...
pub async fn run() -> anyhow::Result<()> {
    println!("\x1b[1mSmellyType macOS Setup Wizard\x1b[0m");
    println!("==========================\n");
    println!("This wizard will set up SmellyType as a native macOS app with proper permissions.\n");
    ...
    if app_exists {
        print_success("SmellyType.app already exists");
        ...
        println!(
            "SmellyType needs to be installed as an app bundle for proper macOS integration.\n"
        );
        println!("This will:");
        println!("  - Create /Applications/SmellyType.app");
    ...
    println!("SmellyType needs microphone access to capture your voice.\n");
    println!(
        "We'll open System Settings and launch SmellyType to trigger the permission prompt.\n"
    );
    ...
    if setup_mic {
        ...
        print_info("Launching SmellyType.app to trigger permission prompt...");
        let app_path = super::app_bundle::app_bundle_path();
        ...
        println!("    2. If no dialog appears, find 'SmellyType' in the list and toggle it ON");
        println!(
            "    3. If SmellyType isn't in the list, press the hotkey once to trigger the prompt"
        );
        println!();
    ...
    println!("SmellyType needs Input Monitoring permission to capture global hotkeys.\n");
    ...
    if setup_input {
        ...
        println!("    3. Select 'SmellyType.app'");
    ...
    println!("SmellyType needs Accessibility permission to type transcribed text.\n");
    ...
    println!("SmellyType can show notifications when transcription completes.\n");
    ...
    if setup_notifications {
        ...
        println!();
        println!("    Find 'SmellyType' in the list and configure notification settings.");
        println!();
    ...
    println!("SmellyType supports two methods for global hotkey capture:\n");
    ...
    println!("SmellyType can start automatically when you log in via Login Items.\n");
    ...
    let (use_parakeet, model) = if has_parakeet {
        println!("SmellyType supports two speech recognition engines:\n");
        ...
        println!("SmellyType uses Whisper for speech recognition.\n");
    ...
    println!("Your SmellyType installation is ready. Here's a summary:\n");

    println!("  App bundle:     /Applications/SmellyType.app");
    if use_hammerspoon {
    ...
    println!("\n\x1b[1mStarting SmellyType...\x1b[0m\n");
    ...
    let _ = tokio::process::Command::new("open")
        ...
        .output()
        .await;
    print_success("SmellyType.app started");

    println!();
    println!("Press {} to start recording!", hotkey);
    println!();
    println!("Useful commands:");
    println!("  smellytype status            - Check daemon status");
    println!("  smellytype status --follow   - Watch status in real-time");
    println!("  smellytype setup app-bundle --status  - Check app bundle status");
    println!("  smellytype record toggle     - Toggle recording from CLI");
