//! GPU backend management for smellytype
//!
//! Supports two installation modes:
//! 1. Tiered mode (DEB/RPM pre-built): Multiple CPU binaries (avx2, avx512) + vulkan in /usr/lib/smellytype/
//! 2. Simple mode (AUR source build): Native CPU binary at /usr/bin/smellytype + vulkan in /usr/lib/smellytype/
//!
...
//! backend may select the wrong GPU by default. Use SMELLYTYPE_VULKAN_DEVICE environment
//! variable to select a specific GPU:
//!   - SMELLYTYPE_VULKAN_DEVICE=nvidia  (selects NVIDIA GPU)
//!   - SMELLYTYPE_VULKAN_DEVICE=amd     (selects AMD GPU)
//!   - SMELLYTYPE_VULKAN_DEVICE=intel   (selects Intel GPU)
//!
...
const SMELLYTYPE_LIB_DIR: &str = "/usr/lib/smellytype";
const SMELLYTYPE_BIN: &str = "/usr/bin/smellytype";
const SMELLYTYPE_BIN_LOCAL: &str = "/usr/local/bin/smellytype";
const SMELLYTYPE_CPU_BACKUP: &str = "/usr/lib/smellytype/smellytype-cpu";
const SMELLYTYPE_NATIVE: &str = "/usr/lib/smellytype/smellytype-native";

/// Get the active smellytype binary path (prefers /usr/bin, falls back to /usr/local/bin)
fn get_active_binary_path() -> &'static str {
    // If /usr/bin/smellytype exists and points somewhere, use it
    if Path::new(SMELLYTYPE_BIN).exists() {
        return SMELLYTYPE_BIN;
    }
    // Fall back to /usr/local/bin/smellytype
    if Path::new(SMELLYTYPE_BIN_LOCAL).exists() {
        return SMELLYTYPE_BIN_LOCAL;
    }
    // Default to standard location
    SMELLYTYPE_BIN
}

/// Resolve the real binary `/usr/bin/smellytype` dispatches to.
///
...
/// wrapper itself, whose filename is just "smellytype", which used to make
...
/// the Arch package shipped smellytype-onnx-migraphx with no migraphx dependency).
///
...
fn unresolved_provider_deps(binary_name: &str) -> Option<Vec<String>> {
    let resolved = fs::canonicalize(Path::new(SMELLYTYPE_LIB_DIR).join(binary_name)).ok()?;
...
fn describe_onnx_variant(name: &str) -> &'static str {
    match name {
        "smellytype-onnx-avx2" | "smellytype-parakeet-avx2" => "ONNX CPU (AVX2)",
        "smellytype-onnx-avx512" | "smellytype-parakeet-avx512" => "ONNX CPU (AVX-512)",
        "smellytype-onnx-cuda-12" => "ONNX GPU (CUDA 12)",
        "smellytype-onnx-cuda-13" => "ONNX GPU (CUDA 13)",
        "smellytype-onnx-cuda" | "smellytype-parakeet-cuda" => "ONNX GPU (CUDA, unversioned)",
        "smellytype-onnx-migraphx" => "ONNX GPU (MIGraphX)",
        "smellytype-onnx-rocm" | "smellytype-parakeet-rocm" => "ONNX GPU (MIGraphX, legacy name)",
...
pub enum Backend {
    Cpu,    // Legacy: smellytype-cpu (deprecated, kept for compatibility)
    Native, // Simple mode: source-built native CPU binary (smellytype-native)
...
impl Backend {
    fn binary_name(&self) -> &'static str {
        match self {
            Backend::Cpu => "smellytype-cpu",
            Backend::Native => "smellytype-native",
            Backend::Avx2 => "smellytype-avx2",
            Backend::Avx512 => "smellytype-avx512",
            Backend::Vulkan => "smellytype-vulkan",
        }
    }
...
fn is_tiered_mode() -> bool {
    Path::new(SMELLYTYPE_LIB_DIR)
        .join("smellytype-avx2")
        .exists()
}
...
fn backend_from_binary_name(name: &str) -> Option<Backend> {
    match name {
        "smellytype-cpu" => Some(Backend::Cpu),
        "smellytype-native" => Some(Backend::Native),
        "smellytype-avx2" => Some(Backend::Avx2),
        "smellytype-avx512" => Some(Backend::Avx512),
        "smellytype-vulkan" => Some(Backend::Vulkan),
        _ => None,
    }
}
...
pub fn detect_current_backend() -> Option<Backend> {
    ...
    // Check if the smellytype binary is a symlink
...
pub fn detect_available_backends() -> Vec<Backend> {
    ...
    if is_tiered_mode() {
        ...
        for backend in [Backend::Avx2, Backend::Avx512, Backend::Vulkan] {
            let path = Path::new(SMELLYTYPE_LIB_DIR).join(backend.binary_name());
            if path.exists() {
        ...
        if Path::new(SMELLYTYPE_NATIVE).exists() {
                ...
                .unwrap_or(false)
        {
            ...
            available.push(Backend::Native);
        } else if Path::new(SMELLYTYPE_CPU_BACKUP).exists() {
            // Legacy backup location
            available.push(Backend::Cpu);
        }

        // Check for vulkan
        if Path::new(SMELLYTYPE_LIB_DIR)
            .join("smellytype-vulkan")
            .exists()
        {
...
/// Parse SMELLYTYPE_VULKAN_DEVICE environment variable and return the appropriate vendor
pub fn get_selected_gpu_vendor() -> Option<GpuVendor> {
    std::env::var("SMELLYTYPE_VULKAN_DEVICE")
        .ok()
...
/// Apply GPU selection environment variables based on SMELLYTYPE_VULKAN_DEVICE
...
fn switch_backend_tiered(backend: Backend) -> anyhow::Result<()> {
    let binary_path = Path::new(SMELLYTYPE_LIB_DIR).join(backend.binary_name());
    let active_bin = get_active_binary_path();
...
fn enable_simple_mode() -> anyhow::Result<()> {
    let vulkan_path = Path::new(SMELLYTYPE_LIB_DIR).join("smellytype-vulkan");
    let native_path = Path::new(SMELLYTYPE_NATIVE);
    let active_bin = get_active_binary_path();

    if !vulkan_path.exists() {
        anyhow::bail!(
            "Vulkan backend not installed.\n\
             The smellytype-vulkan binary was not found in {}",
            SMELLYTYPE_LIB_DIR
        );
    }
    ...
    if let Ok(target) = fs::read_link(active_bin) {
        if target.file_name().map(|n| n.to_str()) == Some(Some("smellytype-vulkan")) {
            anyhow::bail!("GPU backend is already enabled.");
        }
    }

    // Ensure lib dir exists
    fs::create_dir_all(SMELLYTYPE_LIB_DIR)
        .map_err(|e| anyhow::anyhow!("Failed to create {}: {}", SMELLYTYPE_LIB_DIR, e))?;

    // Handle different scenarios:
    // 1. New layout: symlink to smellytype-native -> just update symlink
    ...
    if !is_symlink && Path::new(active_bin).exists() && !native_path.exists() {
        ...
        fs::rename(active_bin, SMELLYTYPE_CPU_BACKUP).map_err(|e| {
            anyhow::anyhow!(
                "Failed to backup CPU binary (need sudo?): {}\n\
                 Try: sudo smellytype setup gpu --enable",
                e
            )
        })?;
    ...
    symlink(&vulkan_path, active_bin).map_err(|e| {
        ...
        if native_path.exists() {
            ...
            let _ = fs::rename(SMELLYTYPE_CPU_BACKUP, active_bin);
        }
        anyhow::anyhow!(
...
fn disable_simple_mode() -> anyhow::Result<()> {
    ...
    let use_backup_layout = Path::new(SMELLYTYPE_CPU_BACKUP).exists();
        ...
        fs::remove_file(active_bin).map_err(|e| {
            anyhow::anyhow!(
                "Failed to remove symlink (need sudo?): {}\n\
                 Try: sudo smellytype setup gpu --disable",
                e
            )
        })?;
    }
    ...
    if use_native_layout {
        // New layout: create symlink to smellytype-native
        ...
        fs::rename(SMELLYTYPE_CPU_BACKUP, active_bin).map_err(|e| {
            anyhow::anyhow!(
...
pub fn show_status() {
    println!("=== SmellyType Backend Status ===\n");
    ...
    if is_parakeet {
        ...
        if let Some(target) = detect_active_parakeet_backend() {
            ...
            // `target` comes from /usr/bin/smellytype, which describes the next
            ...
            match (&running_path, daemon) {
                (Some(path), Some(pid)) => {
                    ...
                    if running_name != target {
                        ...
                        println!(
                            "    {}",
                            Path::new(SMELLYTYPE_LIB_DIR).join(&target).display()
                        );
                        println!("  Restart smellytype to pick it up:");
                        println!("    systemctl --user restart smellytype");
                    }
        ...
        // now, and separately what /usr/bin/smellytype would launch next. The
    ...
    if is_parakeet {
        ...
        let onnx_backends = [
            (
                "smellytype-onnx-avx2",
                "smellytype-parakeet-avx2",
                "ONNX CPU (AVX2)",
            ),
            (
                "smellytype-onnx-avx512",
                "smellytype-parakeet-avx512",
                "ONNX CPU (AVX-512)",
            ),
            (
                "smellytype-onnx-cuda-12",
                "smellytype-onnx-cuda",
                "ONNX GPU (CUDA 12)",
            ),
            (
                "smellytype-onnx-cuda-13",
                "smellytype-onnx-cuda-13",
                "ONNX GPU (CUDA 13)",
            ),
            (
                "smellytype-onnx-migraphx",
                "smellytype-onnx-rocm",
                "ONNX GPU (MIGraphX)",
            ),
        ];
        ...
        for (binary, legacy_binary, display) in onnx_backends {
            let path = Path::new(SMELLYTYPE_LIB_DIR).join(binary);
            let legacy_path = Path::new(SMELLYTYPE_LIB_DIR).join(legacy_binary);
    ...
    if gpus.is_empty() {
        ...
        if gpus.len() > 1 {
            ...
            if let Some(selected) = get_selected_gpu_vendor() {
                println!(
                    "GPU selection: {} (via SMELLYTYPE_VULKAN_DEVICE)",
                ...
                println!("  SMELLYTYPE_VULKAN_DEVICE=nvidia   # Use NVIDIA GPU");
                println!("  SMELLYTYPE_VULKAN_DEVICE=amd      # Use AMD GPU");
                println!("  SMELLYTYPE_VULKAN_DEVICE=intel    # Use Intel GPU");
                println!();
                println!(
                    "For systemd, create ~/.config/systemd/user/smellytype.service.d/gpu.conf:"
                );
                println!("  [Service]");
                println!("  Environment=\"SMELLYTYPE_VULKAN_DEVICE=nvidia\"");
            }
        }
    }
    ...
    if is_parakeet {
        ...
        if !is_gpu_active && detect_best_parakeet_gpu_backend().is_some() {
            ...
            println!("  sudo smellytype setup gpu --enable");
        } else if is_gpu_active {
            println!("To switch back to CPU:");
            println!("  sudo smellytype setup gpu --disable");
        }
...
fn detect_best_parakeet_gpu_backend() -> Option<(&'static str, &'static str)> {
    ...
    let find_binary = |new_name: &'static str, legacy_name: &'static str| -> Option<&'static str> {
        if Path::new(SMELLYTYPE_LIB_DIR).join(new_name).exists() {
            Some(new_name)
        } else if Path::new(SMELLYTYPE_LIB_DIR).join(legacy_name).exists() {
            Some(legacy_name)
        } else {
            None
        }
    ...
    if let Some(binary) = find_binary("smellytype-onnx-migraphx", "smellytype-onnx-rocm") {
        if has_amd {
    ...
    if has_nvidia {
        ...
        let cuda_pref: &[&str] = match host_cuda_major {
            Some(13) => &["smellytype-onnx-cuda-13", "smellytype-onnx-cuda"],
            Some(12) => &["smellytype-onnx-cuda-12", "smellytype-onnx-cuda"],
        ...
        for name in cuda_pref {
            if Path::new(SMELLYTYPE_LIB_DIR).join(name).exists() {
...
pub fn enable() -> anyhow::Result<()> {
    ...
    if is_parakeet {
        ...
        let (backend_binary, backend_name) = detect_best_parakeet_gpu_backend().ok_or_else(|| {
            ...
            let hint = if (has_amd || has_nvidia) && !has_avx512 {
                "You have a GPU, but the ONNX GPU binaries (CUDA/MIGraphX) require a CPU with \
                 ...
                 sudo ln -sf /usr/lib/smellytype/smellytype-onnx-avx2 /usr/bin/smellytype\n\n\
                 Or use the Whisper engine with Vulkan GPU acceleration:\n  \
                 smellytype setup onnx --disable && sudo smellytype setup gpu --enable"
            } else if has_amd {
                "You have an AMD GPU. Install smellytype-onnx-migraphx for GPU acceleration."
            } else if has_nvidia {
                "You have an NVIDIA GPU. Install smellytype-onnx-cuda-12 (for CUDA 12.x) or \
                 smellytype-onnx-cuda-13 (for CUDA 13.x) for GPU acceleration."
            } else {
        ...
        println!();
        println!("Restart smellytype to use GPU acceleration:");
        println!("  systemctl --user restart smellytype");
    } else {
    ...
    if is_parakeet {
        ...
        if let Some(backend_name) = best_backend {
            ...
            println!(
                ...
                backend_name
                    .trim_start_matches("smellytype-onnx-")
                    .trim_start_matches("smellytype-parakeet-")
            );
        } else {
            anyhow::bail!(
                "No ONNX CPU backend found.\n\
                 Install smellytype-onnx-avx2 or smellytype-onnx-avx512."
            );
        }
        ...
        println!();
        println!("Restart smellytype to use CPU inference:");
        println!("  systemctl --user restart smellytype");
    } else {
...
fn detect_best_cpu_backend() -> Backend {
    ...
    if let Ok(cpuinfo) = fs::read_to_string("/proc/cpuinfo") {
        if cpuinfo.contains("avx512f") {
            let avx512_path = Path::new(SMELLYTYPE_LIB_DIR).join("smellytype-avx512");
...
fn detect_best_parakeet_cpu_backend() -> Option<&'static str> {
    ...
    if let Ok(cpuinfo) = fs::read_to_string("/proc/cpuinfo") {
        if cpuinfo.contains("avx512f") {
            if let Some(binary) =
                find_binary("smellytype-onnx-avx512", "smellytype-parakeet-avx512")
            {
                return Some(binary);
            }
        }
    ...
    // Fall back to AVX2
    find_binary("smellytype-onnx-avx2", "smellytype-parakeet-avx2")
}

/// Switch to an ONNX backend binary (tiered mode)
fn switch_backend_tiered_parakeet(binary_name: &str) -> anyhow::Result<()> {
    let binary_path = Path::new(SMELLYTYPE_LIB_DIR).join(binary_name);
    let active_bin = get_active_binary_path();

    if !binary_path.exists() {
        anyhow::bail!(
            "ONNX backend not found: {}\n\
             Install the appropriate smellytype-onnx package.",
            binary_path.display()
        );
    }
...
mod tests {
    ...
    fn backend_from_binary_name_rejects_non_variants() {
        ...
        assert_eq!(backend_from_binary_name("smellytype"), None);
        assert_eq!(backend_from_binary_name("smellytype-onnx-migraphx"), None);
