...
//!
//! Every ONNX-engine model smellytype downloads is mirrored to Cloudflare R2 at
...
//!
//! See `roadmap` in `CLAUDE.md` ("smellytype-models CDN") and the matching
...
/// back to the network: `smellytype info models` compares file sizes against it
/// on every listing, and `smellytype info models --verify` re-hashes against it.
/// Dot-prefixed so it can't be mistaken for one of the model's own files.
pub const CACHED_MANIFEST_FILENAME: &str = ".smellytype-manifest.json";
...
/// smellytype started caching them) and "the cached copy is unreadable". Callers
