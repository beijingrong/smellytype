...
/// One entry in the registry consumed by the `smellytype-mirror-registry`
...
/// flat list. Used by `smellytype-mirror-registry` to drive
...
/// Download a model artifact from the smellytype-models R2 mirror.
///
...
///
/// No HuggingFace fallback. SmellyType controls R2 directly so we can serve
...
/// `model_catalog::model_installed` look, so smellytype would report the model as
...
pub async fn interactive_select() -> anyhow::Result<()> {
    println!("SmellyType Model Selection\n");
    println!("=======================\n");
    ...
    if status.map(|s| s.success()).unwrap_or(false) {
        ...
        match restart {
            ...
            _ => {
                ...
                print_info("Restart manually: systemctl --user restart smellytype");
            }
        }
    } else {
        println!("\n---");
...
pub async fn set_model(model_name: &str, restart: bool) -> anyhow::Result<()> {
    ...
    match set_model_route(model_name) {
        SetModelRoute::Parakeet => {
            ...
            if !model_dir.exists() {
                ...
                println!(
                    "\n  Run 'smellytype setup --download --model {}' to download it first.",
                    model_name
                );
        ...
        SetModelRoute::UnsupportedEngine(engine) => {
            ...
            println!(
                "\n  Use: smellytype setup --download --model {} --activate",
                model_name
            );
            println!("  Or switch engines in the TUI: smellytype configure");
        ...
        SetModelRoute::Whisper => {
            ...
            if !model_path.exists() {
                print_failure(&format!("Model '{}' is not installed", model_name));
                println!("\n  Run 'smellytype setup model' to download it first.");
                println!("  Or 'smellytype setup model --list' to see installed models.");
                anyhow::bail!("Model not installed: {}", model_name);
            }
    ...
    if restart {
        ...
        print_info("Restart daemon to use new model: systemctl --user restart smellytype");
        println!(
            "       Or use: smellytype setup model --set {} --restart",
            model_name
        );
    }
...
pub fn list_installed() {
    ...
    if !found && !openvino_found {
        println!("\n  Run 'smellytype setup model' to download a model.");
    }
}
...
fn update_config_model(model_name: &str) -> anyhow::Result<()> {
    if let Some(config_path) = Config::default_path() {
        if config_path.exists() {
            ...
            print_info("No config file found. Run 'smellytype setup' first.");
            Ok(())
        }
    } else {
...
pub fn list_installed_parakeet() {
    ...
    if !found {
        ...
        println!("\n  Run 'smellytype setup model' and select Parakeet to download.");
    }
}
...
///
/// Cohere is the largest artifact smellytype ships (up to ~4 GB across a
...
pub fn download_cohere_model(model_name: &str) -> anyhow::Result<()> {
    ...
    println!(
        "This is the largest model smellytype ships. Ensure you have at least \
         {} MB of free space in {}.\n",
...
async fn handle_cohere_selection(selection: usize) -> anyhow::Result<()> {
    ...
    print_warning(&format!(
        "Cohere is a {} MB download — the largest model smellytype offers.",
        model.size_mb,
    ));
...
pub fn list_installed_moonshine() {
    ...
    if !found {
        ...
        println!("\n  Run 'smellytype setup model' and select Moonshine to download.");
    }
}
...
/// Whisper wins that collision in `smellytype setup --model`.
...
/// SenseVoice names to show for `smellytype setup --model`. Uses the directory
...
pub fn list_installed_sensevoice() {
    ...
    if !found {
        ...
        println!("\n  Run 'smellytype setup model' and select SenseVoice to download.");
    }
}
...
/// `openvino_npu_opted_in`; `smellytype setup npu` persists that opt-in before
...
/// TUI and `smellytype config set`. Like `set_openvino_config`, this only writes
