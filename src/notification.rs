...
fn linux_common_args(lifetime: Lifetime, urgency: Option<&str>) -> Vec<String> {
    let mut args = vec![
        "--app-name=SmellyType".to_string(),
        format!("--expire-time={}", lifetime.expire_ms()),
        "-h".to_string(),
        "string:x-canonical-private-synchronous:smellytype".to_string(),
        "-h".to_string(),
...
///
/// Every SmellyType notification on Linux belongs here. A caller that shells out
...
fn send_macos_native(title: &str, body: &str, engine: Option<TranscriptionEngine>) {
    ...
    let bundled_path =
        "/Applications/SmellyType.app/Contents/Resources/terminal-notifier.app/Contents/MacOS/terminal-notifier";
    ...
    let content_image = engine.and_then(|e| match e {
        TranscriptionEngine::Parakeet => {
            Some("/Applications/SmellyType.app/Contents/Resources/parakeet.png")
        }
        TranscriptionEngine::Whisper => {
            Some("/Applications/SmellyType.app/Contents/Resources/whisper.png")
        }
...
mod linux_tests {
    ...
    fn common_args_carry_identity_and_gnome_hints() {
        ...
        assert!(args.contains(&"--app-name=SmellyType".to_string()));
        ...
        assert!(args.contains(&"string:x-canonical-private-synchronous:smellytype".to_string()));
