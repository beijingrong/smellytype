...
//!
//! Provides a system tray icon showing smellytype status with a context menu
...
/// Current smellytype state
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SmellyTypeState {
    Idle,
    Recording,
    Transcribing,
    Stopped,
...
impl SmellyTypeState {
    fn from_str(s: &str) -> Self {
        match s.trim().to_lowercase().as_str() {
            "idle" => SmellyTypeState::Idle,
            "recording" => SmellyTypeState::Recording,
            "transcribing" => SmellyTypeState::Transcribing,
            _ => SmellyTypeState::Stopped,
        }
    }

    fn icon(&self) -> &'static str {
        match self {
            SmellyTypeState::Idle => "🎙",
            SmellyTypeState::Recording => "🔴",
            SmellyTypeState::Transcribing => "⏳",
            SmellyTypeState::Stopped => "⬛",
        }
    }

    fn status_text(&self) -> &'static str {
        match self {
            SmellyTypeState::Idle => "Status: Ready",
            SmellyTypeState::Recording => "Status: Recording...",
            SmellyTypeState::Transcribing => "Status: Transcribing...",
            SmellyTypeState::Stopped => "Status: Daemon not running",
        }
    }
}
...
fn read_state_from_file(path: &PathBuf) -> SmellyTypeState {
    std::fs::read_to_string(path)
        .map(|s| SmellyTypeState::from_str(&s))
        .unwrap_or(SmellyTypeState::Stopped)
}

/// Get the smellytype binary path
fn get_smellytype_path() -> PathBuf {
    std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|d| d.join("smellytype")))
        .filter(|p| p.exists())
        .unwrap_or_else(|| PathBuf::from("smellytype"))
}

/// Execute smellytype command
fn smellytype_cmd(args: &[&str]) {
    let smellytype_path = get_smellytype_path();
    let _ = std::process::Command::new(smellytype_path)
        .args(args)
        .spawn();
}

/// Execute smellytype command and wait for completion
fn smellytype_cmd_wait(args: &[&str]) -> bool {
    let smellytype_path = get_smellytype_path();
    std::process::Command::new(smellytype_path)
        .args(args)
        .status()
        .map(|s| s.success())
        .unwrap_or(false)
...
fn is_autostart_enabled() -> bool {
    ...
    let plist = home.join("Library/LaunchAgents/io.smellytype.daemon.plist");
    plist.exists()
}
...
fn set_model(model_name: &str, is_parakeet: bool) -> bool {
    if is_parakeet {
        smellytype_cmd_wait(&["setup", "parakeet", "--set", model_name])
    } else {
        smellytype_cmd_wait(&["setup", "model", "--set", model_name])
    }
}
...
fn restart_daemon() {
    ...
    let _ = std::process::Command::new("launchctl")
        .args(["kickstart", "-k", "gui/$(id -u)/io.smellytype.daemon"])
        .status();

    // Fallback: kill and restart
    let _ = std::process::Command::new("pkill")
        .args(["-f", "smellytype daemon"])
        .status();

    std::thread::sleep(Duration::from_millis(500));

    smellytype_cmd(&["daemon"]);
}
...
pub fn run(state_file: PathBuf) -> ! {
    println!("Starting SmellyType menu bar...");
    ...
    match pidlock.acquire() {
        ...
        Err(_) => {
            eprintln!("Error: Another smellytype menubar instance is already running.");
            std::process::exit(1);
        }
    }
    ...
    if !state_file.exists() {
        println!("\nWarning: State file not found. Is the smellytype daemon running?");
        println!("Start it with: smellytype daemon\n");
    }
        ...
        *control_flow = ControlFlow::WaitUntil(Instant::now() + Duration::from_millis(100));
        ...
        if let Ok(event) = menu_channel.try_recv() {
            ...
            match id {
                // Recording controls
                menu_ids::TOGGLE => {
                    smellytype_cmd(&["record", "toggle"]);
                }
                menu_ids::CANCEL => {
                    smellytype_cmd(&["record", "cancel"]);
                }
                ...
                menu_ids::OUTPUT_TYPE => {
                    if set_output_mode(OutputMode::Type) {
                        notify("SmellyType", "Output mode: Type text");
                    }
                }
                menu_ids::OUTPUT_CLIPBOARD => {
                    if set_output_mode(OutputMode::Clipboard) {
                        notify("SmellyType", "Output mode: Copy to clipboard");
                    }
                }
                menu_ids::OUTPUT_PASTE => {
                    if set_output_mode(OutputMode::Paste) {
                        notify("SmellyType", "Output mode: Clipboard + Paste");
                    }
                }
                ...
                menu_ids::AUTOSTART_ENABLE => {
                    if is_autostart_enabled() {
                        // Disable
                        if smellytype_cmd_wait(&["setup", "launchd", "--uninstall"]) {
                            notify("SmellyType", "Auto-start disabled");
                        }
                    } else {
                        // Enable
                        if smellytype_cmd_wait(&["setup", "launchd"]) {
                            notify("SmellyType", "Auto-start enabled");
                        }
                    }
                }
                ...
                menu_ids::VIEW_LOGS => {
                    let home = dirs::home_dir().unwrap_or_default();
                    let log_path = home.join("Library/Logs/smellytype");
                    open_path(log_path.to_str().unwrap_or(""));
                }
                menu_ids::RESTART_DAEMON => {
                    notify("SmellyType", "Restarting daemon...");
                    restart_daemon();
                }
                ...
                _ if id.starts_with(menu_ids::MODEL_PREFIX) => {
                    ...
                    if set_model(model_name, is_parakeet) {
                        notify("SmellyType", &format!("Switched to model: {}", model_name));
                    }
                }
