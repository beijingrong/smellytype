...
mod tests {
    ...
    fn test_word_replacements() {
        let config = make_config(false, &[("vox type", "smellytype")]);
        let processor = TextProcessor::new(&config);

        assert_eq!(
            processor.process("I use vox type for dictation"),
            "I use smellytype for dictation"
        );
    }
    ...
    fn test_combined_processing() {
        let config = make_config(true, &[("smellytype", "SmellyType")]);
        let processor = TextProcessor::new(&config);

        assert_eq!(
            processor.process("I use smellytype period"),
            "I use SmellyType."
        );
    }
